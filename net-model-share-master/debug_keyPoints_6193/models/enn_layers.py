...
def conv3x3(gspace, inplanes, out_planes, stride=1, padding=1, dilation=1, bias=False, fixparams=False, in_type='regular', out_type='regular'):
    """3x3 convolution with padding"""
    in_type = FIELD_TYPE[in_type](gspace, inplanes, fixparams=fixparams)
    out_type = FIELD_TYPE[out_type](gspace, out_planes, fixparams=fixparams)
...
def conv1x1(gspace, inplanes, out_planes, stride=1, padding=0, dilation=1, bias=False, fixparams=False, in_type='regular', out_type='regular'):
    """1x1 convolution"""
    in_type = FIELD_TYPE[in_type](gspace, inplanes, fixparams=fixparams)
    out_type = FIELD_TYPE[out_type](gspace, out_planes, fixparams=fixparams)
...
def convnxn(gspace, inplanes, out_planes, kernel_size=3, stride=1, padding=0, groups=1, bias=False, dilation=1, fixparams=False, in_type='regular', out_type='regular'):
    in_type = FIELD_TYPE[in_type](gspace, inplanes, fixparams=fixparams)
    out_type = FIELD_TYPE[out_type](gspace, out_planes, fixparams=fixparams)
...
def ennAdaptiveAvgPool(gspace, inplanes, output_size):
    in_type = FIELD_TYPE['regular'](gspace, inplanes)
    return enn.PointwiseAdaptiveAvgPool(in_type, output_size=output_size)
