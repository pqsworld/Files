...
def simple_nms_modified(scores, nms_radius: int):
    """ Fast Non-maximum suppression to remove nearby points, 快速非最大值抑制移除附近点 """
    assert (nms_radius >= 0)
...
class DKD(nn.Module):
    def __init__(self, radius=2, top_k=0, scores_th=0.2, n_limit=20000):
        ...
        self.unfold = nn.Unfold(kernel_size=self.kernel_size, padding=self.radius) #该函数时从一个 batch 图片中，提取出滑动的局部区域块，也就是卷积操作中的提取 kernel filter 对应的滑动窗口
