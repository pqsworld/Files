...
class UnSuperPointNet_small_8_theta_MH(torch.nn.Module):
    """ Pytorch definition of SuperPoint Network. """
    def __init__(self, subpixel_channel=1):
        super(UnSuperPointNet_small_8_theta_MH, self).__init__()
        ...
        self.position0 = nn.Sequential(
            Block(kernel_size=3, in_size=int(32), expand_size=int(32), out_size=int(32),
                     nolinear=hswish(), semodule=SeModule(int(32)), stride=1),
            torch.nn.Conv2d(32, 2, kernel_size=1, stride=1, padding=0)
        )
        #Orientation Head
    ...
    def forward(self, x):
        ...
        # Score Head.
        semi_score0 = self.nonlinear(self.score0(x4))
        # Position Head.
        semi_position0 = self.nonlinear(self.position0(x4))
        # theta Head.
        semi_theta0 = self.nonlinear(self.theta0(x4))

        # Score Head.
        semi_score1 = self.nonlinear(self.score1(x4))
        # Position Head.
        semi_position1 = self.nonlinear(self.position1(x4))
        # theta Head.
        semi_theta1 = self.nonlinear(self.theta1(x4))



        semi0 = torch.cat((semi_score0, semi_position0, semi_theta0), dim=1)
        semi1 = torch.cat((semi_score1, semi_position1, semi_theta1), dim=1)

        desc = None

        output = {'semi0': semi0, 'semi1': semi1, 'desc': desc}
        self.output = output
