...
if __name__ == '__main__':
    ...
    #备份yaml
    d_yaml_path = os.path.join(backup_path,"{:s}".format(args.config.split('/')[-1]))
    shutil.copyfile(args.config,d_yaml_path)

    try:
        #备份dataloader
        s_loader_path = "./datasets/{:s}.py".format(config['data']['dataset'])
        d_loader_path = os.path.join(backup_path,"{:s}.py".format(config['data']['dataset']))
        shutil.copyfile(s_loader_path,d_loader_path)

        #备份train_front
        s_front_path = "./{:s}.py".format(config['front_end_model'])
        d_front_path = os.path.join(backup_path,"{:s}.py".format(config['front_end_model']))
        shutil.copyfile(s_front_path,d_front_path)
    except:
        print("Cant backup config!!!")
