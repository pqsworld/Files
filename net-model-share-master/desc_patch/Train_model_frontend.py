...
class Train_model_frontend(object):
    ...
    def dataParallel(self):
        ...
        self.descriptor_net = self.descriptor_net.to(self.device)
        self.net_params = [self.net.parameters()]
        # self.net_params.append(self.detector_net.parameters())
        self.net_params.append(self.descriptor_net.parameters())
    ...
    def loadModel(self):
        ...
        from desc import HardNet_fast_s, HardNet_fast_t
        
        self.descriptor_net = HardNet_fast_s(train_flag=True).to(self.device)
        # pthpath = r'/data/yey/work/6191/unsuperpoint/logs/0423_correct_noise0_sift180_14s_t20-5_93_nonorm_144_mask_e4800_nomatch_extend_ori5dist_cos_wbmask_addFA_9800_256_student_tw1.4T13_noextend/checkpoints/superPointNet_202800_desc.pth.tar'
        # self.descriptor_net.load_state_dict(torch.load(pthpath)['model_state_dict'])
        # self.descriptor_net.eval()
        # self.descriptor_net = QHardNet_small(cfg=ptq_config, train_flag=True).to(self.device)
        # self.descriptor_net.train()

        #蒸馏相关
        # self.descriptor_net_t = HardNet_fast_t(train_flag=False).to(self.device)
        # #方形描述子
        # pthpath = r'/hdd/file-input/yey/temp/desc_patch/logs/0609_correctnoise0_sift180_14t_t20-5_93_nonorm_144_mask_e4800_nomatch_extend_ori5dist_cos_wbmask_addFA_9800_256/checkpoints/superPointNet_500000_desc.pth.tar'
        # #长条形描述子
        # # pthpath = r'/hdd/file-input/wanghl/superPointNet_495600_desc.pth.tar'
        # self.descriptor_net_t.load_state_dict(torch.load(pthpath)['model_state_dict'])
        # self.descriptor_net_t.eval()

        # #冻结某些层
        # name_list = list(self.descriptor_net.named_parameters())
        # t_params_list = list(self.descriptor_net_t.parameters())
        # #这样写才是指针操作，会直接修改self.descriptor_net
        # for param_idx, param_s in enumerate(self.descriptor_net.parameters()):
        #     if int(name_list[param_idx][0].split('.')[1]) < 2:
        #         param_s.requires_grad = False
        #         t_params_list[param_idx].requires_grad = False
        #         param_s[:] = t_params_list[param_idx]
        #     else:
        #         break
        ...
        while self.n_iter < self.max_iter:
            ...
            for i, sample_train in tqdm(enumerate(self.train_loader)):
                ...
                if self.n_iter % self.config["save_interval"] == 0:
                    ...
                    logging.info(
                        "save model: every %d interval, current iteration: %d, consuming: %.2fs",
