...
from utils.utils import getPtsFromHeatmap, filter_points, getPtsFromLabels2D, getPtsFromLabels2D_torch, sample_desc_from_points_torch, getPtsFromHeatmapByCoordinates, getPtsFromHeatmapByNMS
...
def ip_layer(scores, w_size, kernels):
    ...
    exp_map = torch.exp(torch.divide(scores, scores_max_unpool + eps)) - 1*(1.-eps)         # 保证heatmap为0时的占比为0
    ...
    indexes_map = torch.divide(indexes_map, sum_exp_map + eps)  # 介于1-w_size [2, H, W]
...
def grid_indexes_nms_conv(scores, kernels, w_size):
    ...
    indexes_label = F.conv2d(score_map, kernels['indexes_kernel_'+str(w_size)], stride=w_size).to(scores.device)    # [H / h_size, W / w_size]
...
class Train_model_heatmap_cv2_keynet(Train_model_frontend):
    ...
    ** note: the input, output is different from that of SuperPointFrontend
    ...
    def detector_selfsupervised_msip_loss(self, descA, descB, Homo, inv_Homo):  
        ...
        MSIP_sizes = [4, 8]
        MSIP_factor_loss = [8, 2]   # [256.0, 64.0, 16.0, 4.0, 1.0]
        loss_indexes = 0
        ...
        for idx in range(len(MSIP_sizes)):
            win = MSIP_sizes[idx]
            _, weights_visibleA, map_nms = grid_indexes_nms_conv(visibleA_mask, ip_layer_kernels, win)  # 表示每个patch是否含有重叠区域 1:有
            ...
            coordinate_weighting = True     # 调整边界点loss占比
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        ## get the inputs
        imgA, labelA, labelA_sort, valid_mask, imgB, labelB, labelB_sort, valid_mask_H, name, homography, inv_homography, homography_o= (
            sample['image'], 
        ...
        # labelA_3D = labels2Dto3D_sort(
        #     labelA.to(self.device), labelA_sort.to(self.device), cell_size=self.cell_size, add_dustbin=add_dustbin
        # ).float()
        ...
        # labelB_3D = labels2Dto3D_sort(
        #     labelB.to(self.device), labelB_sort.to(self.device), cell_size=self.cell_size, add_dustbin=add_dustbin
        # ).float()
        ...
        if n_iter % tb_interval == 0 or task == "val":
            ...
            if mode == 'nms':
                ...
                pts_A = getPtsFromHeatmapByNMS(heatmapA[0, 0, :, :].unsqueeze(0).unsqueeze(0), self.conf_thresh, self.nms_dist)
                ...
                warped_pts_A = warp_points(pts_A, mat_H[0].squeeze(), device=heatmapA.device)  # 利用变换矩阵变换坐标点
                warped_pts_A, mask_points = filter_points(warped_pts_A, torch.tensor([W, H], device=heatmapA.device), return_mask=True)
                # === get B's heatmap & pts ===
                ...
                pts_B = getPtsFromHeatmapByNMS(heatmapB[0, 0, :, :].unsqueeze(0).unsqueeze(0), self.conf_thresh, self.nms_dist)
            ...
            if lambda_loss > 0:
                desc_A = coarse_descA[0, :, torch.tensor(pts_A_lable[:120, 1]).long(), torch.tensor(pts_A_lable[:120, 0]).long()].transpose(1, 0)
                for i in range(15):
                ...
                desc_B = coarse_descB[0, :, torch.tensor(pts_B_lable[:120, 1]).long(), torch.tensor(pts_B_lable[:120, 0]).long()].transpose(1, 0)
                for i in range(15):
