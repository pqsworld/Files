...
class Train_model_frontend(object):
    ...
    def dataParallel(self):
        ...
        # self.net = nn.DataParallel(self.net, device_ids=[0,1,2,3], output_device=[0,1,2,3])
    ...
    def adamOptim(self, net, lr):
        ...
        optimizer = optim.Adam(net.parameters(), lr=lr, betas=(0.9, 0.999))     # 开始训练之前需要将网络的参数放到优化器里面,使得优化器能够起作用
        return optimizer
    ...
    def loadModel(self):
        ...
        :return:
        """
        model = self.config["model"]["name"]        # UnSuperPointNet
    ...
    def train(self, **options):
        ...
        #     self.optimizer, 10, gamma=0.95, last_epoch=-1)      # 调整学习率机制：每训练10个epoch更新一次参数、last_epoch默认为-1表示从头开始训练，即从epoch=1开始
        self.scheduler = lr_scheduler.StepLR(
            self.optimizer, 5, gamma=0.8, last_epoch=-1)      # 调整学习率机制：每训练10个epoch更新一次参数、last_epoch默认为-1表示从头开始训练，即从epoch=1开始
        torch.autograd.set_detect_anomaly(True) # 自动求导异常侦测
        while self.n_iter < self.max_iter:
            print("epoch: ", epoch)
            epoch += 1
            for i, sample_train in tqdm(enumerate(self.train_loader)):  # tqdm是一个可以显示进度条的模块;enumerate()可以同时遍历列表中的元素及其索引
                # train one sample
                ...
                #         if j > self.config.get("validation_size", 3):       # 如果指定键的值不存在，返回默认值3
                #             break
                # save model
                ...
                if self.n_iter > self.max_iter:
                    ...
                    break
            self.scheduler.step()       # 一个epoch更新一次学习率
    ...
    def getMasks(self, mask_2D, cell_size, device="cpu"):
        ...
        mask_3D_flattened = torch.prod(mask_3D, 1)      # 返回所有元素的乘积 得到的是[64,16,16]这里的64是batch_size,mask是1*16*16
        return mask_3D_flattened

    def get_loss(self, semi, labels3D_in_loss, mask_3D_flattened, device="cpu"):        # 计算损失函数
        """
    ...
    def printLosses(self, losses, task="train"):
        ...
        for element in list(losses):
            ...
            print(task, "-", element, ": ", losses[element].item() if type(losses[element]) is torch.Tensor else losses[element])
    ...
    def get_heatmap(self, semi, det_loss_type="softmax"):
        if det_loss_type == "Focalloss":
            # heatmap = self.flatten_64to1(semi)
            heatmap = self.flattenDetection_unsuper(semi)
        else:
            heatmap = flattenDetection(semi)    # 将3D图转为热力图
        return heatmap

    
    def get_heatmap_unsuperpoint(self, semi_unsuperpoint, Hc, Wc, block_size):
        heatmap_unsuper_2D = torch.zeros(Hc * block_size, Wc * block_size)

        for i in range(semi_unsuperpoint.size(dim=1)):
            for j in range(semi_unsuperpoint.size(dim=2)):
                if(semi_unsuperpoint[2, i, j] > 0):
                    y = (i * block_size + semi_unsuperpoint[0, i, j] * block_size).round().long()
                    x = (j * block_size + semi_unsuperpoint[1, i, j] * block_size).round().long()
                    if (x < 128) and (y < 128):
                        heatmap_unsuper_2D[y, x] = semi_unsuperpoint[2, i, j]    # 16*16 --> 128*128

        return heatmap_unsuper_2D


    '''batch 类型的3D转2D'''
    def flattenDetection_unsuper(self, semi, block_size=8):
        '''
        semi : [batch_size, 3, H / 8, W / 8]
        '''
        # labels_unsuper_2D = torch.zeros(1, 1, semi.shape[2] * block_size, semi.shape[3] * block_size)
        semi_unsuper_2D = None

        for batch_i in range(semi.shape[0]):
            single = semi[batch_i, :, :, :]
            semi_unsuper_2D_single = self.get_heatmap_unsuperpoint(
                single, 
                semi.shape[2],  # 16
                semi.shape[3],  # 16
                block_size=8
                )
            semi_unsuper_2D_single = semi_unsuper_2D_single.unsqueeze(0).unsqueeze(1)
            if batch_i == 0:
                semi_unsuper_2D = semi_unsuper_2D_single
            else:
                semi_unsuper_2D = torch.cat((semi_unsuper_2D, semi_unsuper_2D_single), dim=0)

        return semi_unsuper_2D
