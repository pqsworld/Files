...
def sample_homography_cv(H, W, max_angle=15, n_angles=31):
    scale = 1
...
def sample_homography_np(
        ...
        perspective_amplitude_y=0.1, patch_ratio=0.5, max_angle=pi/6,
...
def scale_homography_torch(H, shape, shift=(-1,-1), dtype=torch.float32):
    ...
    '''完成的操作，相当于normPts函数里的pts = pts/shape*2 - 1，用矩阵的形式，对H矩阵做变换，使得通过新的H矩阵变换的点完成归一化'''
