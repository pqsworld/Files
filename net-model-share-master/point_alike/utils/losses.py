...
def norm_patches(patches):
    ...
    patches = patches.view(-1, 1, patch_size*patch_size)    # [1, xx, 5, 5] -> [xx, 1, 25]
    d = torch.sum(patches, dim=-1).unsqueeze(-1) + 1e-6     # [xx, 1, 1]
    patches = patches/d
    patches = patches.view(-1, 1, patch_size, patch_size)   # [xx, 1, 5, 5]
...
def extract_patch_from_points(heatmap, points, patch_size=5):
    ...
    heatmap = np.pad(heatmap, pad_size, 'constant')     # [132,132]
    # crop it
    patches = []
    ...
    for i in range(points.shape[0]):
        ...
        patch = ext(heatmap, points[i,:].astype(int), patch_size)   # [5,5]
...
def soft_argmax_2d(patches, normalized_coordinates=True):
    ...
    '''fix by Andy, because the lib(torchgeometry) is not in env'''
    import kornia
    # https://kornia.readthedocs.io/en/latest/geometry.subpix.html
    coords = kornia.geometry.subpix.spatial_soft_argmax2d(patches, temperature=torch.tensor(50.0), normalized_coordinates=False)
    return coords
