...
def draw_lines(img, nb_lines=10):
    ...
    for i in range(num_lines):
        ...
        col = get_random_color(background_color)
        thickness = random_state.randint(min_dim * 0.01 + 1, min_dim * 0.02 + 2)
...
def draw_multiple_polygons(img, max_sides=8, nb_polygons=10, **extra):
    ...
    for i in range(nb_polygons):
        num_corners = random_state.randint(3, max_sides)
        min_dim = min(img.shape[0], img.shape[1])
        rad = max(random_state.rand() * min_dim / 2, min_dim / 5)        # 调整圆的大小
        x = random_state.randint(rad, img.shape[1] - rad)  # Center of a circle
        y = random_state.randint(rad, img.shape[0] - rad)
...
def draw_checkerboard(img, max_rows=7, max_cols=7, transform_params=(0.01, 0.02)):
...
def draw_stripes(img, max_nb_cols=6, min_width_ratio=0.2,
                 transform_params=(0.01, 0.05)):
...
def draw_cube(img, min_size_ratio=0.2, min_angle_rot=math.pi / 10,
    ...
    for i in [0, 1, 2]:
        cv.fillPoly(img, [cube[faces[i]].reshape((-1, 1, 2))],
                    col_face)
    thickness = random_state.randint(min_dim * 0.003 +1, min_dim * 0.015+2)
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
