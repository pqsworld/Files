...
class NonMaxSuppression(torch.nn.Module):
    ...
    def __init__(self, thr=0.0, nms_size=5):
        ...
        padding = nms_size // 2
        self.max_filter = F.MaxPool2d(kernel_size=nms_size, stride=1, padding=padding)
        self.thr = thr
...
def simple_nms_modified(scores, nms_radius: int):
    ...
    def max_pool(x):
        ...
        x_indexes_rank = torch.tensor(list(range(x_indexes.shape[-1]*x_indexes.shape[-2])), device=x_indexes.device).view(x_indexes.shape[-2], x_indexes.shape[-1]).repeat(x_indexes.shape[0], x_indexes.shape[1], 1, 1)  
    ...
    def detect_keypoints(self, scores_map, sub_pixel=True):
        ...
        if sub_pixel:
            ...
            for b_idx in range(b):
                ...
                try:
                    assert indices_kpt.shape[0] >= compensation_point_num
                except:
                    rand_indices = torch.randperm(patch.shape[0])[:(compensation_point_num - indices_kpt.shape[0])].to(patches.device)
                    indices_kpt = torch.cat((indices_kpt, rand_indices), dim=-1)
                ...
                # xy_residual_all.append(xy_residual)
        else:
            for b_idx in range(b):
                indices_kpt = indices_keypoints[b_idx]  # one dimension vector, say its size is M
                keypoints_xy_nms = torch.stack([indices_kpt % w, indices_kpt // w], dim=1)  # Mx2
                ...
                kptscores.append(kptscore)
                # xy_residual_all.append(None)

        return keypoints, scoredispersitys, kptscores   # , xy_residual_all
...
class DKDR(nn.Module):
    ...
    def detect_keypoints(self, scores_map, sub_pixel=True):
        ...
        if sub_pixel:
            ...
            for b_idx in range(b):
                ...
                xy_residual_mid = (xy_residual + self.radius) / (self.radius * 2)    # [-self.radius, self.radius]->[0, 2*self.radius]->[0, 1]
                
                keypoints.append(keypoints_xy)
                scoredispersitys.append(scoredispersity)
                kptscores.append(kptscore)
                xy_residual_all.append(xy_residual_mid)
        else:
            for b_idx in range(b):
                indices_kpt = indices_keypoints[b_idx]  # one dimension vector, say its size is M
                keypoints_xy_nms = torch.stack([indices_kpt % w, indices_kpt // w], dim=1)  # Mx2
    ...
    def forward(self, scores_map, descriptor_map, sub_pixel=False):
        ...
        # 点（w,h） range:(-1~1, -1~1)，第二项用于计算dispersity peak loss， 点对应分数
        keypoints, scoredispersitys, kptscores, xy_residual = self.detect_keypoints(scores_map,
                                                                       sub_pixel)
        if descriptor_map is not None:
            descriptors = sample_descriptor(descriptor_map, keypoints, sub_pixel)
        else:
        ...
        # descriptors: B M D
        # scoredispersitys:
        return keypoints, descriptors, kptscores, scoredispersitys, xy_residual
