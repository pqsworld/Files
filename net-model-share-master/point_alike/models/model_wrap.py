...
def get_heatmap_unsuperpoint(semi_unsuperpoint, Hc, Wc, block_size):
...
'''batch 类型的3D转2D'''
def flattenDetection_unsuper(semi, block_size=8):
    '''
    ...
    for batch_i in range(semi.shape[0]):
        single = semi[batch_i, :, :, :]
        semi_unsuper_2D_single = get_heatmap_unsuperpoint(
            single, 
            semi.shape[2],  # 16
            semi.shape[3],  # 16
            block_size=8
...
class SuperPointFrontend_torch(object):
    ...
    ** note: the input, output is different from that of SuperPointFrontend
    ...
    def __init__(self, config, weights_path, nms_dist, conf_thresh, nn_thresh,
        ...
        self.border_remove = 0  # 暂时不用Remove points this close to the border.
        self.sparsemap = None
        self.heatmap = None
        self.pts = None
    ...
    def loadModel(self, weights_path):
        ...
        if trained:
            ...
            #     model = 'SuperPointNet'
            #     params = {}
            model = self.config['model']['name']        # SuperPointNet_gauss2
    ...
    def soft_argmax_points(self, pts, patch_size=5):
        ...
        import torch
        patches = np.stack(patches)     # 叠加维度 [point_number, 5, 5]
        patches_torch = torch.tensor(patches, dtype=torch.float32).unsqueeze(0)     # [1, point_number, 5, 5]

        # norm patches
        patches_torch = norm_patches(patches_torch)     # [xx, 1, 5, 5]
    ...
    def getPtsFromHeatmap(self, heatmap, bord=0):
        '''
        :param self:
        :param heatmap:
            np (H, W)
    ...
    def getPtsFromLabels2D(self, lables_2D):
        '''
        :param self:
        :param heatmap:
            np (H, W)
        ...
        xs, ys = np.where(lables_2D == 1)  # Confidence threshold.
        ...
        pts[0, :] = ys # abuse of ys, xs
        pts[1, :] = xs
        pts[2, :] = lables_2D[xs, ys]  # check the (x, y) here
    ...
    def run(self, inp, onlyHeatmap=False, train=True):
        ...
        # flatten detection
        # heatmap = flattenDetection_unsuper(semi, block_size=8)   # [1,1,128,128]
        ...
        dense_desc = nn.functional.interpolate(coarse_desc, scale_factor=(self.cell, self.cell), mode='bilinear')   # 稀疏描述子转为稠密描述子
        # norm the descriptor
        def norm_desc(desc):
