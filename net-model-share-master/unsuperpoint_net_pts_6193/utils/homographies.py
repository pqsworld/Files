...
def sample_homography_cv(H, W, max_angle=15, n_angles=25, flip=False, translation=False):
    scale = 1
    n_angles=max_angle + 10
    angles = np.linspace(-max_angle, max_angle, num=n_angles)

    flip_flag = random.choice([0,1])
    if flip and flip_flag:
        angles = angles + 180
    # angles = [-90, -85, -80, -75, -70, -65, -60, 60, 65, 70, 75, 80, 85, 90]
    ...
    valid = np.arange(len(angles))
    idx = valid[np.random.randint(valid.shape[0], size=1)].squeeze().astype(int)
    homo = M[idx]
    
    if translation:
        homo_trans = np.zeros_like(homo)
        homo_trans[0,0] = 1
        homo_trans[1,1] = 1
        homo_trans[2,2] = 1
        homo_trans[0,2] = random.randint(-5, 5)
        homo_trans[1,2] = random.randint(-15, 15)

        homo = homo_trans @ homo
