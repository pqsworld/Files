...
def load_as_float(path): 
    ...
    if h == 122 and w == 36:
        image_temp = np.zeros((128,40))
        image_temp[3:-3,2:-2] = image_np
        image_np = image_temp
    if h == 144 and w == 52:
...
class Read_txt2data_93_mask_expand(data.Dataset):
    """
    """
        ...
        **config,
        ...
        for dataset_name in datasets_93:
            ...
            for sample_num, (A, N_A, B, N_B, M_H, P_A, P_B) in enumerate(zip(
                ...
                A_mask = A_mask.replace('_extend.bmp','_mask.bmp')
                B_mask = B_mask.replace('_extend.bmp','_mask.bmp')
    ...
    def __getitem__(self, index):
        ...
        border_mask[5:-5,4:-4] = 1 #图像128*40 ，实际118*32
        ...
        warped_img_mask = self.inv_warp_image(imgA_mask.squeeze(), homography_centorcrop(homography, 0, -6), mode='bilinear').unsqueeze(0)
