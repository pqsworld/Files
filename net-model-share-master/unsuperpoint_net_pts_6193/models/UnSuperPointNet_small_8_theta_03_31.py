...
class UnSuperPointNet_small_8_theta_03_31(torch.nn.Module):
    """ Pytorch definition of SuperPoint Network. """
    def __init__(self, subpixel_channel=1):
        super(UnSuperPointNet_small_8_theta_03_31, self).__init__()
        ...
        model += [Residual(inp_dim = int(8), out_dim = int(16), k=3, stride=2)]
        model += [Residual(inp_dim = int(16), out_dim = int(32), k=3, stride=2)]
        model += [Residual(inp_dim = int(32), out_dim = int(32), k=3, stride=1)]
        ...
        self.block_theta = Block(kernel_size=3, in_size=int(32), expand_size=int(32), out_size=int(32),
                     nolinear=hswish(), semodule=SeModule(int(32)), stride=1)
        # self.bnPa_score = nn.BatchNorm2d(c5)
        self.convPb_theta = torch.nn.Conv2d(32, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        semi = torch.cat((semi_score, semi_position, semi_theta), dim=1)
