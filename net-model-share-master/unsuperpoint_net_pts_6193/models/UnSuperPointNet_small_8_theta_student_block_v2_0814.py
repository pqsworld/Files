...
class Residual_3(nn.Module):
    def __init__(self, inp_dim, out_dim, k=3, stride=1):
        ...
        self.conv1 = nn.Conv2d(inp_dim, out_dim, (k, k), padding=(p, p), stride=(stride, stride), groups=out_dim // 4, bias=False)
        self.bn1   = nn.BatchNorm2d(out_dim)
        self.relu1 = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv2d(out_dim, out_dim, (k, k), padding=(p, p), groups=out_dim // 4, bias=False)        
        self.bn2   = nn.BatchNorm2d(out_dim)        
...
class UnSuperPointNet_small_8_theta_student_block_v2_0814(torch.nn.Module):
    """ Pytorch definition of SuperPoint Network. """
    def __init__(self, subpixel_channel=1):
        super(UnSuperPointNet_small_8_theta_student_block_v2_0814, self).__init__()
        ...
        model += [Residual_3(inp_dim = int(4), out_dim = int(4), k=3, stride=1)]
        model += [Residual_3(inp_dim = int(4), out_dim = int(4), k=3, stride=1)]
        model += [Residual_3(inp_dim = int(4), out_dim = int(8), k=3, stride=1)]
        model += [Residual_3(inp_dim = int(8), out_dim = int(16), k=3, stride=1)]
        model += [Residual_3(inp_dim = int(16), out_dim = int(16), k=3, stride=1)]

        model += [Block(kernel_size=3, in_size=int(16), expand_size=int(16), out_size=int(16),nolinear=hswish(), semodule=SeModule(int(16)), stride=2)]
        model += [Block(kernel_size=3, in_size=int(16), expand_size=int(16), out_size=int(16),nolinear=hswish(), semodule=SeModule(int(16)), stride=2)]
        ...
        # Detector Head Score.
        self.block_score = Block(kernel_size=3, in_size=int(16), expand_size=int(16), out_size=int(16),
                     nolinear=hswish(), semodule=SeModule(int(16)), stride=1)
        # self.bnPa_score = nn.BatchNorm2d(c5)
        self.convPb_score = torch.nn.Conv2d(16, 1, kernel_size=1, stride=1, padding=0)
        # self.bnPb_score = nn.BatchNorm2d(1)
        # Detector Head Position.
        self.block_position = Block(kernel_size=3, in_size=int(16), expand_size=int(16), out_size=int(16),
                     nolinear=hswish(), semodule=SeModule(int(16)), stride=1)
        # self.bnPa_position = nn.BatchNorm2d(c5)
        self.convPb_position = torch.nn.Conv2d(16, 2, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # theta Head.
        # cPa_theta = self.block_theta(x4)
        # semi_theta = self.nonlinear(self.convPb_theta(cPa_theta))
