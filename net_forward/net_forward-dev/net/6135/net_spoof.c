...
********************************************************************/
static int sl_fingerprint_spoof_6135( float* src, short h, short w, int* score )
{
    int ret = 0;
    int size = w * h * 5;
    int hc = h, wc = w;
    ...
    if ( NULL == memory ) {
        // not enough memory
        SL_LOGE( "sl_fingerprint_spoof_6135 mslloc failed!" );
        return -2;
    }
...
#if NEON
        conv_nxn_single_bias_neon(layer1_image_out, para_conv1_weights + i * input_channel * ksize * ksize,
                                  para_conv1_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
#else
        conv_nxn_single_bias(layer1_image_out, para_conv1_weights + i * input_channel * ksize * ksize,
                             para_conv1_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
#endif
    ...
    for ( i = 0; i < output_channel; i++ ) {
#if NEON
        conv_nxn_single_bias_neon(keep_memory, para_conv2_weights + i * input_channel * ksize * ksize,
                                  para_conv2_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
#else
        conv_nxn_single_bias(keep_memory, para_conv2_weights + i * input_channel * ksize * ksize,
                             para_conv2_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
#endif
        ReLU( conv_memory, relu_memory, hc * wc );
        maxpooling_stride2_size2( relu_memory, hc, wc, pool_memory + i * imsize2 );
    ...
    for ( i = 0; i < output_channel; i++ ) {
#if NEON
        conv_nxn_single_bias_neon(keep_memory, para_conv3_weights + i * input_channel * ksize * ksize,
                                  para_conv3_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
#else
        conv_nxn_single_bias(keep_memory, para_conv3_weights + i * input_channel * ksize * ksize,
                             para_conv3_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
#endif
        ReLU( conv_memory, relu_memory, hc * wc );
        maxpooling_stride2_size2( relu_memory, hc, wc, pool_memory + i * imsize2 );
    ...
    for ( i = 0; i < output_channel; i++ ) {
#if neon
        conv_nxn_single_bias_neon(keep_memory, para_conv4_weights + i * input_channel * ksize * ksize,
                                  para_conv4_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
#else
        conv_nxn_single_bias(keep_memory, para_conv4_weights + i * input_channel * ksize * ksize,
                             para_conv4_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
#endif
        ReLU( conv_memory, relu_memory, hc * wc );
        maxpooling_stride2_size2( relu_memory, hc, wc, pool_memory + i * imsize2 );
    ...
    for ( i = 0; i < output_channel; i++ ) {
        conv_nxn_single_bias(keep_memory, para_conv5_weights + i * input_channel * ksize * ksize,
                             para_conv5_bias[i], ksize, input_channel, conv_memory, &hc, &wc, 0);
    ...
    fullyconnect(keep_memory, hc, wc, fc_input_channel, class_number, para_fc_weights, para_fc_bias, new_memory);
...
int spoof_check(unsigned char* src, const int width, const int height, int enable_learn, int enroll_flag, float* template_feature, int* score)
{
    UNUSED(enable_learn);
    UNUSED(enroll_flag);
    UNUSED(template_feature);

    get_version_spoof();

    SL_LOGE( "spoof_check35 init, image w = %d,h = %d", width, height );

    if (NULL == src || width <= 0 || height <= 0 || score == NULL) {
        SL_LOGE( "spoof_check35 param error" );
        return -1;
    }
    ...
    if ( NULL == out_resize ) {
        SL_LOGE( "spoof_check35 malloc out_resize fail" );
        ret = -1;
        goto close;
    }
    ...
    for ( i = 0; i < width * height; i++ ) {
        out_resize[i] = (float)(src[i]);
    }

    ret = sl_fingerprint_spoof_6135( out_resize, height, width, score );

    if ( ret < 0 ) {
        SL_LOGE( "spoof_check35 error ret %d", ret );
        goto close;
    }
...
close:
    ...
    SL_LOGD( "spoof_check35 out score = %d", *score );
