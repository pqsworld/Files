...
int mistouch_check(unsigned char* img, int width, int height, int* score)
{
    ...
    SL_LOGE("mistouch_check35 init, image w = %d,h = %d", width, height);

    if (NULL == img || width <= 0 || height <= 0 || score == NULL) {
        SL_LOGE("mistouch_check35 param error");
        return -1;
    }
    ...
    if (NULL == out_resize) {
        SL_LOGE("mistouch_check35 malloc out_resize fail");
        ret = -1;
        goto close;
    }
    ...
    if (ret < 0) {
        SL_LOGE("mistouch_check35 error ret %d", ret);
        goto close;
    }
...
close:
    ...
    SL_LOGE("mistouch_check35 out score = %d", *score);
