...
static int get_feature_distance( float* feature, float* template_feature, float* distance )
{
    ...
    float temp1 = 0;
    float temp2 = 0;
    temp1 = ( feature[0] - template_feature[0] ) * ( feature[0] - template_feature[0] );
    temp2 = ( feature[1] - template_feature[1] ) * ( feature[1] - template_feature[1] );
    *distance = SL_Sqrt( temp1 + temp2 );
    return 0;
}
...
int spoof_check( unsigned char* src, const int width, const int height, int enable_learn, int enroll_flag, float* template_feature, int* score )
{
    ...
    SL_LOGE( "spoof_check59 init, image w = %d,h = %d,e_l=%d,e_f=%d", width, height, enable_learn, enroll_flag );

    if ( src == NULL || template_feature == NULL || score == NULL || height < 0 || width < 0 || enable_learn < 0 || enroll_flag < 0 ) {
        return -1;
    }

    if ( height != 160 || width != 32 || !src ) {
        return -1;
    }

    float thre_distance = 0.7;
    ...
    SL_LOGE( "spoof_check59 score=%d,num_exceed_distance=%d", *score, num_exceed_distance );
    return ret;
}
