...
********************************************************************/
static int sl_fingerprint_forward( float* src, short h, short w, int* score )
{
    int ret = 0;
    int size = w * h * 5;
    int hc = h, wc = w;
    ...
    if ( NULL == memory ) {
        // not enough memory
        SL_LOGE( "sl_fingerprint_forward mslloc failed!" );
        return -2;
    }
...
int mistouch_check( unsigned char* img, int width, int height, int* score )
{
    ...
    SL_LOGE( "mistouch_check91 init, image w = %d,h = %d", width, height );

    if ( NULL == img || width <= 0 || height <= 0 || score == NULL ) {
        SL_LOGE( "mistouch_check91 param error" );
        return -1;
    }
    ...
    if ( NULL == out_resize ) {
        SL_LOGE( "mistouch_check91 malloc out_resize fail" );
        ret = -1;
        goto close;
    }
    ...
    ret = sl_fingerprint_forward( out_resize, height, width, score );

    if ( ret < 0 ) {
        SL_LOGE( "mistouch_check91 error ret %d", ret );
        goto close;
    }
...
close:
    ...
    SL_LOGE( "mistouch_check91 out score = %d", *score );
