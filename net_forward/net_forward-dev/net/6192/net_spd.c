...
static NetSpdShort get_para_spd()
{
...
static int net_forward_spd(unsigned char* img, int h, int w, NetSpdShort net, int* score)
{
    ...
    // memory block
    float* memory = ( float* )malloc( 7 * size * sizeof( float ) );
...
/*
int spd_check_char( unsigned char* img, int width, int height, int* score )
{
#if BENCHMARK
    double start = get_current_time();
#endif
    get_version_spd();
    SL_LOGE( "spd_check92 init, image w = %d,h = %d", width, height );

    if ( NULL == img || width <= 0 || height <= 0 || score == NULL ) {
        SL_LOGE( "spd_check92 param error" );
        return -1;
    }

    int ret = 0;

    ret = net_forward_spd( img, height, width, get_para_spd(), score );

    if ( ret < 0 ) {
        SL_LOGE( "spd_check92 error ret %d", ret );
    }

    SL_LOGE( "spd_check92 out score = %d", *score );
#if BENCHMARK
...
*/
int spd_check(int* src, const int height, const int width, float* out)
{
#if BENCHMARK
    double start = get_current_time();
#endif
