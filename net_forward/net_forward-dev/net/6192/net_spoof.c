...
int spoof_check( unsigned char* src, const int width, const int height, int enable_learn, int enroll_flag, float* template_feature, int* score )
{
    ...
    SL_LOGE( "spoof_check92 init, image w = %d,h = %d,e_l=%d,e_f=%d", width, height, enable_learn, enroll_flag );

    if ( src == NULL || template_feature == NULL || score == NULL || height < 0 || width < 0 || enable_learn < 0 || enroll_flag < 0 ) {
        SL_LOGE("spd_check92 param error");
    }

    float thre_distance = 0.1;
    ...
    SL_LOGE( "spoof_check92 score=%d,num_exceed_distance=%d", *score, num_exceed_distance );
#if BENCHMARK
