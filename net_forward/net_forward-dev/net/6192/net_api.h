...
extend "C" {
    ...
    JLW_EXPORTS int ori_exp(unsigned char* src, const int h, const int w, unsigned char* dst, const int exth, const int extw);
    /*输入输出同一个尺寸，src是扩边后的图，dst是增强后的图*/
    JLW_EXPORTS int net_enhance(unsigned char* src, const int h, const int w, unsigned char* dst);
