...
# import sys
# sys.path.append('/data/yey/work/desc_clear/e2cnn-master/')
...
class BTL(nn.Module):
    def __init__(self, hadama_dim):
        super(BTL, self).__init__()
        # self.fc = nn.Linear(in_channel, out_channel)
        self.hadama_dim = hadama_dim
        self.hadama_trans = torch.tensor(hadamard(hadama_dim)).float()

    def thresholding_desc(self, descs):
        norm = torch.sqrt(torch.sum(descs * descs, dim = 1)) * 0.2
        norm = norm.detach().int().unsqueeze(-1).expand_as(descs).float()
        descs = torch.where(descs < norm, torch.sqrt(descs), torch.sqrt(norm))  # .int() 
        return descs
    
    def get_hadama_quant_desc(self, descs):
        descs_thr = self.thresholding_desc(descs * 5000 + 5000)
        wht_descs = torch.einsum('nd,de->ne', descs_thr, self.hadama_trans.to(descs_thr.device))  # .int() 
        return wht_descs

    def forward(self, feature):
        feature0 = self.get_hadama_quant_desc(feature[:, :128])
        feature1 = self.get_hadama_quant_desc(feature[:, 128:])
        ...
        return feature              # B x D  {-1, 1} float()
...
class Block_Deform(nn.Module):
    ...
    def forward(self, x):
        out = self.nolinear1(self.bn1(self.conv1(x)))
        out = self.nolinear2(self.bn2(self.conv2(out)))
        # out, out_extern_offset = self.conv2(out, extern_offset)
        ...
        out = out + self.shortcut(x) if (self.stride == 1 and x.shape[1] == out.shape[1]) else out
        return out
        # return out, out_extern_offset

class Block_Group(nn.Module):
    '''expand + depthwise + pointwise'''

    def __init__(self, kernel_size, in_size, expand_size, out_size, nolinear, semodule, stride, first=False):
        super(Block_Group, self).__init__()
        self.stride = stride
        if first:
            self.conv1 = P4ConvZ2(in_size, expand_size,
                                kernel_size=1, stride=1, padding=0, bias=False)
        else:
            self.conv1 = P4ConvP4(in_size, expand_size,
                                kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(expand_size)
        self.nolinear1 = nolinear
        self.conv2 = P4ConvP4(expand_size, expand_size, kernel_size=kernel_size,
                               stride=stride, padding=kernel_size // 2, groups=expand_size, bias=False)
        self.bn2 = nn.BatchNorm2d(expand_size)
        self.nolinear2 = nolinear
        self.conv3 = P4ConvP4(expand_size, out_size,
                               kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3 = nn.BatchNorm2d(out_size)
        self.se = semodule
        self.shortcut = nn.Sequential()
        ...
        if stride == 1 and in_size == out_size:
            self.shortcut = nn.Sequential(
                P4ConvP4(in_size, out_size, kernel_size=1,
                          stride=1, padding=0, bias=False),
                nn.BatchNorm2d(out_size),
            )
...
# class Ehswish(enn.ReLU):
#     def __init__(self, in_type: enn.FieldType, inplace: bool = False):
       
#         super(Ehswish, self).__init__(in_type, inplace)

#     def forward(self, input: enn.GeometricTensor) -> enn.GeometricTensor:    
#         assert input.type == self.in_type, "Error! the type of the input does not match the input type of this module"
#         return enn.GeometricTensor(input.tensor * F.relu6(input.tensor + 3, inplace=self._inplace) / 6, self.out_type)
...
class HardNet_fast_twice_big_BTL(nn.Module):
    """HardNet model definition
    """
    def __init__(self, train_flag=False):
        super(HardNet_fast_twice_big_BTL, self).__init__()
        self.train_flag = train_flag
        ...
        self.BTL_module = BTL(hadama_dim=128) 
    ...
    def forward(self, input, angle=0):
        ...
        # # print('x_features.shape: ', x_features.shape)
        # x = x_features.view(x_features.size(0), -1)
        return self.BTL_module(L2Norm()(x)) / 16
...
class HardNet_fast_twice_big_MO(nn.Module):
    ...
    def forward(self, input, angle=0):
        ...
        return L2Norm()(x) # , x_features_expand.transpose(2, 3).contiguous().view(b, -1, fh, fw)
...
class HardNet_fast_big(nn.Module):
    ...
    def forward(self, input, angle=0):
        ...
        # x_fea = None
        if angle == 1:
            x_features_trans = torch.flip(x_features_trans, dims=[1])
            x = x_features_trans.view(x_features_trans.size(0), -1)
            # x_features_expand = torch.flip(x_features_trans, dims=[1])
            # x = x_features_expand.view(x_features_expand.size(0), -1)
        else:
...
class HardNet_fast_big_Deform(nn.Module):
    """HardNet model definition
    """
    def __init__(self, train_flag=False):
        super(HardNet_fast_big_Deform, self).__init__()
        self.train_flag = train_flag
        ...
        self.features = nn.Sequential(
            ...
            # Block_Pconv(kernel_size=3, in_size=int(32), expand_size=int(32), out_size=int(32),
            #          nolinear=hswish(), semodule=SeModule(int(32)), stride=1),
            Block_Deform(kernel_size=3, in_size=int(64), expand_size=int(64), out_size=int(64),
                     nolinear=hswish(), semodule=SeModule(int(64)), stride=1),
            # Block_Pconv(kernel_size=3, in_size=int(32), expand_size=int(32), out_size=int(32),
            #          nolinear=hswish(), semodule=SeModule(int(32)), stride=1),
            Block_Deform(kernel_size=3, in_size=int(64), expand_size=int(128), out_size=int(128),
                     nolinear=hswish(), semodule=SeModule(int(128)), stride=1),
            # Block_Pconv(kernel_size=3, in_size=int(32), expand_size=int(32), out_size=int(32),
            #          nolinear=hswish(), semodule=SeModule(int(32)), stride=1),
            nn.Conv2d(128, 8, kernel_size=1, padding=0)
...
class HardNet_fast_twice_half3_MO(nn.Module):
    ...
    def __init__(self, train_flag=False):
        ...
        self.features = nn.Sequential(
            ...
            Block_noshortcut(kernel_size=3, in_size=int(8), expand_size=int(16), out_size=int(16),
                     nolinear=hswish(), semodule=SeModule(int(16)), stride=2),
            MobileOne(in_planes=16, planes=16, stride=1, use_se=True, num_conv_branches=4, inference_mode=False, nolinear=hswish()),
            MobileOne(in_planes=16, planes=16, stride=1, use_se=True, num_conv_branches=4, inference_mode=False, nolinear=hswish()),
            MobileOne(in_planes=16, planes=16, stride=1, use_se=True, num_conv_branches=4, inference_mode=False, nolinear=hswish()),
            # Block_noshortcut(kernel_size=3, in_size=int(16), expand_size=int(16), out_size=int(16),
            #          nolinear=hswish(), semodule=SeModule(int(16)), stride=1),
...
class HardNet_fast_Group(nn.Module):
    """HardNet model definition
    """
    def __init__(self, train_flag=False):
        super(HardNet_fast_Group, self).__init__()
        self.train_flag = train_flag
       
        self.features = nn.Sequential(
            Block_Group(kernel_size=3, in_size=int(1), expand_size=int(8), out_size=int(8),
                     nolinear=hswish(), semodule=None, stride=2, first=True),
            Block_Group(kernel_size=3, in_size=int(8), expand_size=int(16), out_size=int(16),
                     nolinear=hswish(), semodule=None, stride=2),
            Block_Group(kernel_size=3, in_size=int(16), expand_size=int(32), out_size=int(32),
                     nolinear=hswish(), semodule=None, stride=1),
            Block_Group(kernel_size=3, in_size=int(32), expand_size=int(32), out_size=int(32),
                     nolinear=hswish(), semodule=None, stride=1),
            Block_Group(kernel_size=3, in_size=int(32), expand_size=int(32), out_size=int(32),
                     nolinear=hswish(), semodule=None, stride=1),
            Block_Group(kernel_size=3, in_size=int(32), expand_size=int(64), out_size=int(64),
                     nolinear=hswish(), semodule=None, stride=1),
            P4MConvP4M(64, 8, kernel_size=1, padding=0)
        )
...
# class HardNet_fast_Ecnn_twice_patch(nn.Module):
#     """HardNet model definition
#     """
#     def __init__(self, dropout_rate=0.5, train_flag=False,
#                  N: int = 4,
...
#         # nStages = [4, 8, 16, 16, 16, 32]
#         nStages = [8, 16, 32, 64] # [8, 16, 32, 32, 32, 64]
#         self.in_type = [r1] + [FIBERS[main_fiber](self.gspace, p, fixparams=self._fixparams) for p in nStages[:-1]]     # [0, 2, 5, 11, 11, 11, 22] -> [0, 1, 2, 4, 4, 4, 8]
#         self.out_type = [FIBERS[main_fiber](self.gspace, p, fixparams=self._fixparams) for p in nStages[:-1]]    # [1, 2, 5, 5, 5, 11]
...
class HardNet_fast_deform_offset(nn.Module):
    """HardNet model definition
    """
    def __init__(self, train_flag=False):
        super(HardNet_fast_deform_offset, self).__init__()
        self.train_flag = train_flag
       
        self.features = nn.Sequential(
            Block_Deform(kernel_size=3, in_size=int(1), expand_size=int(8), out_size=int(8),
                     nolinear=hswish(), semodule=SeModule(int(8)), stride=2, offset=False),
            Block_Deform(kernel_size=3, in_size=int(8), expand_size=int(16), out_size=int(16),
                     nolinear=hswish(), semodule=SeModule(int(16)), stride=2),
            Block_Deform(kernel_size=3, in_size=int(16), expand_size=int(32), out_size=int(32),
                     nolinear=hswish(), semodule=SeModule(int(32)), stride=1),
        ...
        self.downdim = nn.Conv2d(64, 8, kernel_size=1, padding=0)
    ...
    def forward(self, input, angle=0):
        count = 0
        for m_b in self.features:
            if count == 0:
                x_features, x_offset = m_b(self.input_norm(input), None)
            else:
                # print(m_b.stride, x_offset.shape)
                x_offset = x_offset[:, :, ::m_b.stride, ::m_b.stride]             # 下采样?
                x_features, x_offset = m_b(x_features, x_offset)
            count += 1
        x_features = self.downdim(x_features)
        del x_offset
        # x_features = self.features(self.input_norm(input))      # bx8x34x10
        b, o, dh, dw = x_features.shape
        # print('x_features.shape: ', x_features.shape
...
class HardNet_fast_deform_one(nn.Module):
    """HardNet model definition
    """
    def __init__(self, train_flag=False):
        super(HardNet_fast_deform_one, self).__init__()
        self.train_flag = train_flag
