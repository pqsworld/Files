    ...
    def match_loss(image_a_pred, image_b_pred, matches_a, matches_b, M=1.0, 
        ...
        if method == '2d':
            ...
            def sampleDescriptors(image_a_pred, matches_a, mode, norm=False):
                image_a_pred = image_a_pred.unsqueeze(0) # torch [1, D, H, W] [1,64,16,16]
                matches_a.unsqueeze_(0).unsqueeze_(2)   # [1,1000,1,2]
                ...
                if norm:
                    dn = torch.norm(matches_a_descriptors, p=2, dim=1) # 此处即为L2-normalizes（归一化），得到统一长度的描述子，Compute the norm of b_descriptors
