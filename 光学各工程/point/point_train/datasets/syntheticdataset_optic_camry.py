...
class syntheticdataset_optic_camry(data.Dataset):
    """
    """
    ...
    def __init__(
        ...
        **config,
        ...
        self.flag6159 = False
        old_root_path = 'E:\\80x64database\\train\\'
        if self.flag6159:   # 6159图库
            ...
            lib_gallery = ['camry-snm',
                '20240305_omgs3_wxn_dry', 
                #'snm', 
                '20240221_omgs3_tm_dfx1', 
                '20240304_omgc2_dw', 
                '20240122_omgc2_cw', 
                '20240301_omgs3_tm_dfx1',
                #'sun_1', 
                '20240122_omgc2_dw', 
                #'dry',
                '20240221_omgs3_tm_cdw', 
                #'dw',
                '20240301_omgs3_tm_cw',
                '20240221_omgs3_tm_cw', 
                '20240304_omgs3_tm_cdw',
                '20240305_omgs3_wxn_dw',
                'camry-msm', 
                'camry-wet', 
                '20240125_omgc2_cdw', 
                #'cw', 
                ...
                #'rotate_belly', 
                '20240125_omgc2_dfx1', 
                '20240305_omgs3_wxn_dfx1', 
                '20240122_omgc2_dry', 
                '20240304_omgs3_tm_dfx1', 
                '20240301_omgs3_tm_cdw', 
                #'sun_2', 
                #'ghm_msm', 
                #'sun_1_1', 
                '20240305_omgs3_wxn_cw', 
                'camry-lgm',
                'oppo_hepburn_boe_dry',
                'oppo_hepburn_boe_dw',
                'oppo_hepburn_boe_normal',
                'oppo_hepburn_boe_partpress_1_2',
                'oppo_hepburn_boe_partpress_3_4',
                'oppo_hepburn_boe_powder',
                'oppo_hepburn_boe_rotateBelly',
                'oppo_hepburn_boe_rotateTips',
                'oppo_hepburn_wxn_dry',
                'oppo_hepburn_wxn_normal',
                'oppo_hepburn_wxn_partpress_1_2',
                'oppo_hepburn_wxn_partpress_3_4',
                'oppo_hepburn_wxn_powder'
                ]                                 
            


            root_path = '/ssd/share/optic_point/OppoCamryTrainData/' # '/home/linwc/match/data/6193/process9800/'
        ...
        for lib_name in lib_gallery:
            ...
            if self.newTrans:
                print('Use New Trans!!!')
                match_path = Path(root_path + 'new-trans/' + lib_name + '/trans.txt')      # 'trans_info_choose.txt' 'trans'
                orientation_poor_path = Path(root_path + 'new-trans/' + lib_name + '/orientation_poor.npy')
            else:
                match_path = Path(root_path + lib_name + '/trans_choose.txt')      # 'trans_info_choose.txt' 'trans'
            ...
            if self.flag6159:    
                ...
                #print(im1_list[0])
                im1_list = [i[8:].replace("\\\\","\\").replace('.bmp', '_4.bmp') for i in im1_list]
            # image1_list.extend([i + '_enhance' for i in im1_list])
            im2_list = df['path'].to_list()
            im2_list = [root_path+i[7:].replace("\\\\","\\").replace('.bmp', '_4.bmp') for i in im2_list]
            #print("PATH1_____________",im2_list[0])
            ...
            for i in range(len(im2_list)):
                ...
                path_share = im2_list[i].rsplit("\\",3)[0]
                im1_list[i] = path_share + '/' + im1_list[i] 
            #print("PATH2___________",im1_list[0])
            ...
            if os.path.exists(point_path):
                ...
                for line in zip(df_p[0]):  # line: [index, .]
                    if line[0][0:4] == '/hdd':
                        ...
                        img_6159_path.append(line[0][0:].replace(old_root_path, root_path))
            ...
            # 方向一致性
            if os.path.exists(orientation_poor_path):
                ban_orientation = np.load(orientation_poor_path, allow_pickle=True)
                print('ban_orientation num: ', ban_orientation.shape)
    ...
    def __getitem__(self, index):
        ...
        def get_rand_negtives(img_path):
            ...
            r_s = 0
            r_s_f = 0
            finger_list = ["L0", "L1", "R0", "R1"]
            while r_s < r_N:
                #r_f = random.randint(0, 3)
                r_n = str(random.randint(0, 100)).zfill(4)
                img_path_split = img_path.split('/')
                #r_f = (r_f + int(finger_list.index(img_path_split[-2]) == r_f)) % 4
                #img_ne_path = "/".join(img_path_split[:-2]) + '/{:s}'.format(finger_list[r_f])
                img_ne_path = "/".join(img_path_split[:-2])
                path_del=os.listdir(img_ne_path)
                path_del.remove(img_path_split[-2])
                img_ne_path = os.path.join(img_ne_path,random.choice(path_del))
                sample=random.choice(os.listdir(img_ne_path))
                sample=sample.replace("_mask.bmp","_4.bmp").replace("_1.bmp","_4.bmp").replace("_2.bmp","_4.bmp")
                img_ne_path=os.path.join(img_ne_path,sample)
                #img_ne_path = "/".join(img_path_split[:-2]) + '/{:s}/{:4s}_4.bmp'.format(finger_list[r_f], r_n)
                try:
                    assert img_ne_path[:-10] != img_path[:-10]
                except:
                    print(img_ne_path)
                    print(img_path)
                    exit()
        ...
        kpt_shape = None
        if self.newLabels:
            imgA = load_as_float(sample_i['image'][0].replace("\\","/"))
            imgB = load_as_float(sample_i['image'][1].replace("\\","/"))
            kpt_imgA = load_as_float(sample_i['kpt_img'][0].replace("\\","/"))
            kpt_imgB = load_as_float(sample_i['kpt_img'][1].replace("\\","/"))
            kpt_shape = kpt_imgA.shape

            # img_ne = deepcopy(imgA)
            # img_ne2 = deepcopy(imgB)
            # img_ne_path = deepcopy(sample_i['image'][0])
            # img_ne2_path = deepcopy(sample_i['image'][1])
            img_ne, img_ne_path = get_rand_negtives(sample_i['image'][0].replace("\\","/"))
            img_ne2, img_ne2_path = get_rand_negtives(sample_i['image'][1].replace("\\","/")) 

            imgA_part_mask = load_as_float(sample_i['image'][0].replace('_4.bmp', '_mask.bmp').replace("\\","/"))  # 122 x 36
            imgB_part_mask = load_as_float(sample_i['image'][1].replace('_4.bmp', '_mask.bmp').replace("\\","/"))
            img_ne_part_mask = load_as_float(img_ne_path.replace('_4.bmp', '_mask.bmp').replace("\\","/"))
            img_ne2_part_mask = load_as_float(img_ne2_path.replace('_4.bmp', '_mask.bmp').replace("\\","/"))
        ...
        if self.transform is not None:
            ...
            if kpt_shape == (122, 30):
                imgA = F.pad(imgA, pad=(2, 2, 0, 0), mode="constant", value=0)     # 128 x 48 -> 128 x 52
                imgB = F.pad(imgB, pad=(2, 2, 0, 0), mode="constant", value=0)     # 128 x 48 -> 128 x 52
                kpt_imgA = F.pad(kpt_imgA, pad=(3, 3, 0, 0), mode="constant", value=0)     # 122 x 30 -> 122 x 36
                kpt_imgB = F.pad(kpt_imgB, pad=(3, 3, 0, 0), mode="constant", value=0)     # 122 x 30 -> 122 x 36

                img_ne = F.pad(img_ne, pad=(2, 2, 0, 0), mode="constant", value=0)     # 128 x 48 -> 128 x 52
                img_ne2 = F.pad(img_ne2, pad=(2, 2, 0, 0), mode="constant", value=0)     # 128 x 48 -> 128 x 52

                imgA_part_mask = F.pad(imgA_part_mask, pad=(3, 3, 0, 0), mode="constant", value=0)     # 122 x 30 -> 122 x 36
                imgB_part_mask = F.pad(imgB_part_mask, pad=(3, 3, 0, 0), mode="constant", value=0)     # 122 x 30 -> 122 x 36
                img_ne_part_mask = F.pad(img_ne_part_mask, pad=(3, 3, 0, 0), mode="constant", value=0)     # 122 x 30 -> 122 x 36
                img_ne2_part_mask = F.pad(img_ne2_part_mask, pad=(3, 3, 0, 0), mode="constant", value=0)     # 122 x 30 -> 122 x 36
        ...
        # [1, 136, 32]
        if Wdev_left <= 0:
            H, W = imgA.shape[1], imgA.shape[2]    # 80 x 64

            # 旋转增强
            homography_en = self.sample_homography(H, W, max_angle=15)    # 80 x 64
            ...
            imgA_ext = self.inv_warp_image(imgA.squeeze(), homography_en, mode='bilinear').unsqueeze(0)                              # [80, 64]
           
            homography_en_kpt = deepcopy(homography_en) # homography_centorcrop(homography_en, 3, 8)            # -> [80, 64]
            homography_en = deepcopy(homography_en) # homography_centorcrop(homography_en, 4, 6)            # -> [80, 64]
            ...
            imgB_crop = kpt_imgB.squeeze().unsqueeze(0)    # 80 X 64 

            imgA = deepcopy(imgA_ext)   # imgA_ext[0, 4:-4, 6:-6].unsqueeze(0)  

            imgB_ext = deepcopy(imgB)               # cut -> [80, 64] 
            # imgB = imgB[0, 4:-4, 6:-6].unsqueeze(0)   # [80, 64]

            img_ne_ext = deepcopy(img_ne)               # cut -> [80, 64] 
            # img_ne = img_ne[0, 4:-4, 6:-6].unsqueeze(0)   # [80, 64]

            img_ne2_ext = deepcopy(img_ne2)               # cut -> [80, 64] 
            # img_ne2 = img_ne2[0, 4:-4, 6:-6].unsqueeze(0)   # [80, 64]

            # imgA_part_mask = F.pad(imgA_part_mask, pad=(-Wdev_left, -Wdev_left), mode="constant", value=0)  # pad -> [80, 64]
            ...
            imgA_part_mask_ori = deepcopy(imgA_part_mask)   # imgA_part_mask * imgA_point_mask       # 80x64
        ...
        H, W = imgA.shape[1], imgA.shape[2]     # 80, 64
        # print(H, W)
        # print(H, W)
        self.H = H
        self.W = W
        ...
        if kpt_shape == (122, 30):
            homography = homography_centorcrop(homography, 0, -3)     # [122, 30]->[122, 36]
        
        # homography = homography_centorcrop(homography, Hdev_top + 1, Wdev_left)     # [122, 36]->[120, 40]
        homography_ext = deepcopy(homography)# homography_centorcrop(homography, Hdev_top - 4, Wdev_left - 4)     # [80, 64]
        
        imgA_ext_T = self.inv_warp_image(imgA_ori.squeeze(), homography_ext, mode='bilinear').unsqueeze(0)      # [80, 64]
        ...
        inv_homography_ext = deepcopy(inv_homography) # homography_centorcrop(inv_homography, Hdev_top - 4, Wdev_left - 4)  # [80, 64]
        imgB_ext_T = self.inv_warp_image(imgB_ori.squeeze(), inv_homography_ext, mode='bilinear').unsqueeze(0)      # [80, 64]
        ...
        homography_o = deepcopy(homography) # homography_centorcrop(homography, -Hdev_top, -Wdev_left)    # [80, 64]
        ...
        if 'point' in self.samples[index]:
            ...
            if kpt_shape == (122, 36):        
                ...
                pnt_max_num = 220
                warped_pnts_norm = torch.zeros((pnt_max_num, 2)).type(torch.FloatTensor)
                pntsB_norm = torch.zeros((pnt_max_num, 2)).type(torch.FloatTensor)
                sift_maskA = torch.ones(pnt_max_num).type(torch.FloatTensor)
                sift_maskB = torch.ones(pnt_max_num).type(torch.FloatTensor)
                warped_ori = torch.zeros(pnt_max_num).type(torch.FloatTensor)
                pntB_ori = torch.zeros(pnt_max_num).type(torch.FloatTensor)
            ...
            if kpt_shape == (122, 36): 
                ...
                labels_2D = torch.ones(H, W)
                labels_2D_sort = torch.ones(H, W)
            ...
            if kpt_shape == (122, 36):      
                ...
                labels_2D_H = torch.ones(H, W)
                labels_2D_H_sort = torch.ones(H, W)
