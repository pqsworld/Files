...
class SyntheticDataset_gaussian_cv2_expand93_true_neg_bin2_kpt_big98(data.Dataset):
    """
    """
        ...
        **config,
        ...
        if self.flag6159:   # 6159图库
            ...
            lib_gallery = [
                    ...
                    # '6195-DK7-110-wet-auto',               # 38400
                    # '6195-DK7-110-wet-gain6',
                    # '6195-DK7-140-wet-auto',
                    # '6195-DK7-140-wet-gain6',
                    # '6195-DK7-110-inference-auto',
                    # '6195-DK7-110-normal-auto',
                    # '6195-DK7-110-partialPress-auto',      # 53120
                    # '6195-DK7-110-powder-auto',
                    # '6195-DK7-110-rot-auto'
                    '6250.rotate.p17.s150.1023'

            ]                                           # total:  154953 + 
            root_path = '/ssd/share/6250-point/' # '/home/linwc/match/data/6193/process9800/'
        ...
        for lib_name in lib_gallery:
            ...
            if self.flag6159:    
                ...
                im1_list = [i[7:].rjust(4, '0') + '_kpt' for i in im1_list]
            # image1_list.extend([i + '_enhance' for i in im1_list])
            im2_list = df['path'].to_list()
            im2_list = [i[5:].replace(old_root_path, root_path).replace('.bmp', '_kpt.bmp') for i in im2_list]
            # print(im2_list[0])
    ...
    def __getitem__(self, index):
        ...
        def get_rand_negtives(img_path):
            ...
            while r_s < r_N:
                ...
                img_ne_path = "/".join(img_path_split[:-2]) + '/{:s}/{:4s}_kpt.bmp'.format(finger_list[r_f], r_n)
                try:
        ...
        if self.newLabels:
            ...
            kpt_imgA = load_as_float(sample_i['kpt_img'][0])
            kpt_imgB = load_as_float(sample_i['kpt_img'][1])
            ...
            imgA_part_mask = load_as_float(sample_i['image'][0].replace('_kpt', '_mask'))  # 122 x 36
            imgB_part_mask = load_as_float(sample_i['image'][1].replace('_kpt', '_mask'))
            img_ne_part_mask = load_as_float(img_ne_path.replace('_kpt', '_mask'))
            img_ne2_part_mask = load_as_float(img_ne2_path.replace('_kpt', '_mask'))
