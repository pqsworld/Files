...
class SyntheticShapes(BaseDataset):
    default_config = {
            ...
            'generation': {
                'split_sizes': {'training': 200, 'validation': 2, 'test': 2},
                'image_size': [960, 1280],
                'random_seed': 0,
                'params': {
                    'generate_background': {
    ...
    def _init_dataset(self, **config):
        ...
        primitives = parse_primitives(config['primitives'], self.drawing_primitives)  # 定义magicnet所需数据集
        ...
        for primitive in primitives:
            ...
            for s in splits:
                ...
                f = [p.replace('.bmp', '.npy') for p in f]
