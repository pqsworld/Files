...
class SyntheticDataset_gaussian_cv2_expand93_true_neg_bin2_kpt(data.Dataset):
    """
    """
    ...
    def __init__(
        ...
        **config,
        ...
        # 点图
        kpt_image2_list = [i_n.replace('extend', '_enhanceMergeOld') for i_n in image2_list]
        kpt_image1_list = [i_n.replace('extend', '_enhanceMergeOld') for i_n in image1_list]
    ...
    def __getitem__(self, index):
        ...
        if Wdev_left < 0:
            ...
            homography_en_kpt = homography_centorcrop(homography_en, 4, 8)            # -> [120, 36]
            homography_en = homography_centorcrop(homography_en, 4, 6)            # -> [120, 40]
            imgA_crop = self.inv_warp_image(kpt_imgA[:, 1:-1, :].squeeze(), homography_en_kpt, mode='bilinear').unsqueeze(0) 
            imgB_crop = kpt_imgB[:, 1:-1, :].squeeze().unsqueeze(0)    # 120 X 36 
