...
class SyntheticDataset_gaussian_cv2_expand(data.Dataset):
    """
    """
    ...
    def __init__(
        ...
        **config,
        ...
        # rep2 = '/hdd/file-input/qint/6159_parallel/Test_sys/logs/ori_siftlabels_out/output'
            ...
            im2_list = [i[7:].replace('enhance', 'extend') for i in im2_list]
            # print(im2_list[0])
            ...
            for line in zip(df_p[0]):  # line: [index, .]
                if line[0][0:4] == '/hdd':
                    ...
                    img_6159_path.append(line[0][0:].replace('enhance', 'extend'))
    ...
    def __getitem__(self, index):
        ...
        if self.newLabels:
            ...
            imgA_part_mask = load_as_float(sample_i['image'][0].replace('_extend', '_mask'))  
        ...
        if self.transform is not None:
            imgA = self.transform(imgA)                     # 144 x 52
            imgB = self.transform(imgB)
            imgA_part_mask = self.transform(imgA_part_mask) # 136 x 36
        ...
        # [1, 136, 32]
        H, W = imgA.shape[1], imgA.shape[2]         # 144 x 52
        # print(H, W)
        if Wdev_left < 0:
            ...
            homography_en = self.sample_homography(H, W)    # 144 x 52
            ...
            # imgB = imgPhotometric(imgB.squeeze().numpy())   # imgA: no Photometric imgB: Photometric
            # imgB = torch.tensor(imgB, dtype=torch.float32)
            # imgB = imgB.squeeze().unsqueeze(0)
            
            # imgA = self.inv_warp_image(imgA.squeeze(), homography_en, mode='bilinear').unsqueeze(0)
            imgA_crop = imgA[4:-4, 6:-6, :].squeeze().unsqueeze(0) 

            imgA_ext = self.inv_warp_image(imgA.squeeze(), homography_en, mode='bilinear').unsqueeze(0)                              # [144, 52]
            imgB_ext = deepcopy(imgB)
            homography_en = homography_centorcrop(homography_en, 4, 6)            # -> [136, 40]

            imgA = imgA_ext[0, 4:-4, 6:-6].unsqueeze(0)                 # cut -> [136, 40]
            # print(imgA.shape, imgA[4:-4, 6:-6, :].shape, imgB[0, 4:-4, 6:-6].shape)
            # imgA = self.inv_warp_image(imgA[4:-4, 6:-6, :].squeeze(), homography_en, mode='bilinear').unsqueeze(0)  

            imgB = imgB[0, 4:-4, 6:-6].unsqueeze(0)
            ...
            imgA_point_mask[:, 2:-2, 4:-4] = 1              #  限制点在中心132x32 ?
        ...
        homography_o = homography_centorcrop(homography, -Hdev_top, -Wdev_left)
        ...
        if 'point' in self.samples[index]:
            ...
            if 0:
                ...
                # desc2 = np.array([np.fromiter(p, dtype=int) for p in desc2_list], dtype="float32")
