...
class SyntheticDataset_gaussian_cv2_expand93_true_neg_bin2_debase(data.Dataset):
    """
    """
    ...
    def __init__(
        ...
        **config,
        ...
        splits = {s: {"images": [], "points": [], "names": [], "desc": [], "trans": [], "FA_flag": []} for s in [self.action]}       # self.action = 'train'
        ...
        if self.flag6159:   # 6159图库
            ...
            lib_gallery = [
                    ...
                    '6193_DK7_partialPress_8mul_merge',  # 13906
        ...
        count = -1
        for (A, N_A, B, N_B, M_H, FA_F) in zip(
            files1['imgA'], files1['imgA_name'],
            files1['imgB'], files1['imgB_name'], files1['trans'], files1['FA_flag']):   # zip返回元组
        ...
        for s in splits:
            ...
            if len(splits[s]['desc']) != 0:
                for obj in ["images", "points", "names", "desc", "trans", "FA_flag"]:
                    splits[s][obj] = np.array(splits[s][obj])[perm].tolist()
            else:
                for obj in ["images", "points", "names", "trans", "FA_flag"]:
                    splits[s][obj] = np.array(splits[s][obj])[perm].tolist()
    ...
    def crawl_folders(self, splits):
        ...
        if len(splits[self.action]['desc']) != 0:
            for (img, pnt, name, desc, trans, fa_flag) in zip(
                splits[self.action]["images"], splits[self.action]["points"], splits[self.action]["names"], splits[self.action]["desc"], splits[self.action]["trans"], splits[self.action]["FA_flag"]
            ):
                sample = {"image": img, "point": pnt, "name": name, "desc": desc, "trans": trans, "FA_flag": fa_flag}
                sequence_set.append(sample)
        else:
            for (img, name, pnt, trans, fa_flag) in zip(
                splits[self.action]["images"], splits[self.action]["names"], splits[self.action]["points"], splits[self.action]["trans"], splits[self.action]["FA_flag"]
            ):
                # if names[-7:] == "enhance":
                sample = {"image": img, "point": pnt, "name": name, "trans": trans, "FA_flag": fa_flag}
                sequence_set.append(sample)
    ...
    def __getitem__(self, index):
        ...
        def get_rand_negtives(img_path):
            ...
            while r_s < r_N:
                ...
                try:
                    img_ne = load_as_float(img_ne_path.replace('_extend', '').replace('process9800', 'process'))
                    r_s += 1
                except:
                    r_s_f += 1
        ...
        if self.newLabels:
            imgA = load_as_float(sample_i['image'][0].replace('_extend', '').replace('process9800', 'process'))
            imgB = load_as_float(sample_i['image'][1].replace('_extend', '').replace('process9800', 'process'))            
        ...
        # [1, 136, 32]
        if Wdev_left < 0:
            imgA = F.pad(imgA.unsqueeze(0), pad=(10, 10, 5, 5), mode="constant", value=0).squeeze(0)     # 118 x 32 -> 128 x 52
            imgB = F.pad(imgB.unsqueeze(0), pad=(10, 10, 5, 5), mode="constant", value=0).squeeze(0)      # 118 x 32 -> 128 x 52
            img_ne = F.pad(img_ne.unsqueeze(0), pad=(10, 10, 5, 5), mode="constant", value=0).squeeze(0)     # 118 x 32 -> 128 x 52
            img_ne2 = F.pad(img_ne2.unsqueeze(0), pad=(10, 10, 5, 5), mode="constant", value=0).squeeze(0)      # 118 x 32 -> 128 x 52
            
            H, W = imgA.shape[1], imgA.shape[2]    # 128 x 52

            # 旋转增强
            homography_en = self.sample_homography(H, W)    # 128 x 52
            ...
            imgA_part_mask *= imgA_point_mask       # 120x40
