...
class SyntheticDataset_gaussian_cv2_expand93_true_neg_bin2_rect(data.Dataset):
    """
    """
    ...
    def __getitem__(self, index):
        ...
        if Wdev_left < 0:
            ...
            # 计算trans角度        
            mid_homography = np.array(sample_i["trans"])
            mid_homography = torch.tensor(mid_homography.reshape(2, 3), dtype=torch.float32)
            mid_homography = 2 * mid_homography / 512
            mid_vec_one = torch.tensor([[0, 0, 1]], dtype=torch.float32)
            mid_homography = torch.cat((mid_homography, mid_vec_one), 0)    #  3x3

            mid_trans_obj = AffineTransform(matrix=mid_homography.cpu().numpy())
            rotate_theta = -mid_trans_obj.rotation * 180 / math.pi # [-180, 180]
            rotate_trans = cv2.getRotationMatrix2D((W / 2, H / 2), rotate_theta, 1)
            rotate_trans = torch.from_numpy(np.vstack((rotate_trans, [0, 0, 1.]))).float()

            # 旋转增强
            homography_en = self.sample_homography(H, W, max_angle=20)    # 15 128 x 52
            homography_en = torch.tensor(homography_en, dtype=torch.float32)

            homography_en = homography_en @ rotate_trans
