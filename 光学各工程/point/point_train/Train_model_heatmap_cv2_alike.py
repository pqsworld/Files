...
class Train_model_heatmap_cv2_alike(Train_model_frontend):
    ...
    ** note: the input, output is different from that of SuperPointFrontend
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        semiA, coarse_descA, desc_mapA, scoresA, scores_mapA, scoredispersitysA = outA["keypoints"], outA["descriptors"], outA["descriptor_map"], outA["scores"], outA["scores_map"], outA["scoredispersitys"]
        outB = self.net(imgB.to(self.device), sub_pixel=True)
        semiB, coarse_descB, desc_mapB, scoresB, scores_mapB, scoredispersitysB = outB["keypoints"], outB["descriptors"], outB["descriptor_map"], outB["scores"], outB["scores_map"], outB["scoredispersitys"]
        ...
        loss_det_selfsupervised, loss_group = self.detector_selfsupervised_loss_dkd(
                            semiA, 
                            semiB, 
                            coarse_descA,
                            coarse_descB,
        ...
        if n_iter % tb_interval == 0 or task == "val":
            ...
            try: 
                ...
                pred_descA = coarse_descA[0][topk_indices_A, :]
                pred_descB = coarse_descB[0][topk_indices_B, :]
