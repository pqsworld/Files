...
def slransac_project(data, model_class, min_samples, residual_threshold,
           is_data_valid=None, is_model_valid=None,
           max_trials=100, stop_sample_num=np.inf, stop_residuals_sum=0,
           stop_probability=1, random_state=None, initial_inliers=None, weight_pairs=None, angles=None):
    ...
    dis_sum1[range(num_samples), range(num_samples)] = 1   #对角线防止除0
    scale2_matrix = dis_sum0 / dis_sum1
    scale2_matrix = np.triu(scale2_matrix)  #triu 上三角 tril 下三角 （k= 位移）
    mask = (scale2_matrix >= 5/6) * (scale2_matrix <= 6/5)   #true false 索引
    maskac = mask.repeat(num_samples,axis = 0)
    maskac = maskac.reshape(num_samples,num_samples,num_samples)  #[a,1,c]
    maskab = mask.repeat(num_samples,axis = 1).reshape(num_samples,num_samples,num_samples) #[a,b,1]
    maskbc = np.array([mask.flatten()]).repeat(num_samples,axis = 0).reshape(num_samples,num_samples,num_samples)  #[1,b,c]
    dist_mask = maskac * maskab * maskbc
    maskab = np.triu(maskab, k = 1)
    list_ab = np.transpose(maskab.nonzero())[:max_trials]     #用ab距离先筛选出max_trials组
    dist_mask_after_ab = dist_mask[list_ab[:, 0],list_ab[:, 1], list_ab[:, 2]]   #用ac、bc距离进一步筛选组合
    list_abc = list_ab[dist_mask_after_ab]

    angle_diff = get_angle_diff(angles)
    pi_coef = 3.1415926
    sample_angles_diff = angle_diff[list_abc]
    sample_angles_diff = np.abs(sample_angles_diff - np.mean(sample_angles_diff,axis = 1)[:,None])
    sample_angles_diff_mask = np.max(sample_angles_diff,axis = 1) > pi_coef/12    #误差不超过+-15度
    sample_angles_check = list_abc[~sample_angles_diff_mask]

    for idx in sample_angles_check:
        # do sample selection according data pairs
        #spl_idxs = idx
        ...
        # optional check if random sample set is valid
        # if is_data_valid is not None and not is_data_valid(angle_diff, idx): 
        #     continue
