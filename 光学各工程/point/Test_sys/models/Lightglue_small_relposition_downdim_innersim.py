...
def attention_res(query, key, value, nn_sim, res_lam, res_bias, negative_slope):
    dim = query.shape[1]
    scores = torch.einsum('bdhn,bdhm->bhnm', query, key) / dim**.5
    if nn_sim is not None:
        scores = scores + F.leaky_relu(res_lam * nn_sim.unsqueeze(1).expand(-1, scores.shape[1], -1, -1) + res_bias, negative_slope=negative_slope)
    prob = F.softmax(scores, dim=-1)
    return torch.einsum('bhnm,bdhm->bdhn', prob, value), prob
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, d_model: int):
        ...
        self.res_lam = nn.Parameter(torch.ones([1, num_heads, 1, 1]))
        self.res_bias = nn.Parameter(torch.zeros([1, num_heads, 1, 1]))
        self.negative_slope = 0.1

    def forward(self, query, key, value, encoding, nn_sim):
        ...
        # Res Attention
        x, _ = attention_res(query, key, value, nn_sim, self.res_lam, self.res_bias, self.negative_slope)
...
class AttentionalPropagation(nn.Module):
    ...
    def forward(self, x, source, encode, nn_sim):
        message = self.attn(x, source, source, encode, nn_sim)
...
class LightAttentionalGNN(nn.Module):
    def __init__(self, feature_dim: int, layer_names: list, head_num: int):
        ...
        self.depth_confidence = 0.9 # 0.95 # -1   # early stopping, disable with -1 
        self.width_confidence = [0.968777, 0.925089, 0.90828]    # point pruning, disable with -1
        self.match_pair_ratio = 0.5
    ...
    def confidence_threshold(self, layer_index: int) -> float:
        ...
        threshold = 0.75 + 0.1 * np.exp(-4.0 * layer_index / self.n_layers)
    ...
    def check_if_stop_match(self,
                    confidences0: torch.Tensor,
                    confidences1: torch.Tensor,
                    layer_index: int, scores: torch.Tensor) -> torch.Tensor:
        pred_match = self.find_match_idx_score(scores[:, :-1, :-1], 0)
        matches_mask0 = pred_match["matches0"] != -1
        matches_mask1 = pred_match["matches1"] != -1
        
        """ evaluate stopping condition"""
        confidences = torch.cat([confidences0[matches_mask0], confidences1[matches_mask1]], -1)
        threshold = self.confidence_threshold(layer_index)
        pos = 1.0 - (confidences < threshold).float().sum() / (matches_mask0.sum().float() + matches_mask1.sum().float())
        match_ratio = (matches_mask0.sum().float() + matches_mask1.sum().float()) / (confidences0.shape[-1] + confidences1.shape[-1])
        # print(match_ratio, pos)
        return (pos > self.depth_confidence) and (match_ratio > self.match_pair_ratio)
    ...
    def forward(self, desc0, desc1, encoding0, encoding1, nn_sim):
        ...
        for i, (layer, name) in enumerate(zip(self.layers, self.names)):
            if name == 'cross':
                src0, src1 = desc1, desc0
                encode0, encode1 = None, None
                nn_sim0, nn_sim1 = nn_sim, nn_sim.transpose(1, 2)
            else:  # if name == 'self':
                src0, src1 = desc0, desc1
                encode0, encode1 = encoding0, encoding1
                nn_sim0, nn_sim1 = None, None
            delta0, delta1 = layer(desc0, src0, encode0, nn_sim0), layer(desc1, src1, encode1, nn_sim1)
            desc0, desc1 = (desc0 + delta0), (desc1 + delta1)
            ...
            if cal_point_score:
                ...
                scores, _, scores0, scores1 = self.log_assignment[i].forward_all_nn(desc0, desc1, nn_sim)    #.forward_all_nn(desc0, desc1, nn_sim)
                scores_all.append(scores)
                scores0_all.append(scores0)
                scores1_all.append(scores1)
                if do_early_stop:
                    if self.check_if_stop_match(token0, token1, (i - 1) / 2, scores):
                        desc0, desc1 = desc0.transpose(1, 2), desc1.transpose(1, 2)
                        break
                ...
                if do_point_pruning and i in self.prune_layer:
                    ...
                    nn_sim = nn_sim[(mask0[:, :, None] * mask1[:, None, :]) == 1].unsqueeze(0).view(-1, mask0.sum(), mask1.sum())
                    prune0[:, ind0] += 1
                    prune1[:, ind1] += 1
        ...
        if do_point_pruning:            
            ...
            pre_prune_mask0, pre_prune_mask1 = (prune0 < (i - 1) / 2).squeeze(), (prune1 < (i - 1) / 2).squeeze()   # (prune0 < 4).squeeze(), (prune1 < 4).squeeze()
        ...
        scores_final_dict = {}
        scores_final_dict.update({'scores_final': scores_final})
        scores_final_dict.update({'layer_final': (i - 1) / 2})

        return scores_final_dict # scores_final # scores_all[-1]   # , scores0_all, scores1_all
...
class Lightglue_small_relposition_downdim_innersim(nn.Module):
