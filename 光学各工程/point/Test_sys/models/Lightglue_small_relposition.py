...
class LightAttentionalGNN(nn.Module):
    def __init__(self, feature_dim: int, layer_names: list):
        ...
        self.width_confidence = 0.99    # point pruning, disable with -1
        self.last_pruning_thr = 0.1     # 0.5
    ...
    def forward(self, desc0, desc1, encoding0, encoding1):
        ...
        cal_point_score = self.width_confidence > 0
        # 是否剪枝
        ...
        for i, (layer, name) in enumerate(zip(self.layers, self.names)):
            ...
            if cal_point_score:
                ...
                if do_point_pruning:
                    mask0 = self.get_pruning_mask(token0, scores0, (i - 1) / 2, 1 - self.width_confidence)  #.unsqueeze(-1).repeat(1, 1, fea_dim)
                    mask1 = self.get_pruning_mask(token1, scores1, (i - 1) / 2, 1 - self.width_confidence)  #.unsqueeze(-1).repeat(1, 1, fea_dim)
        ...
        scores_final = deepcopy(scores) # torch.zeros((1, m + 1, n + 1)).to(scores.device)
        ...
        if do_point_pruning:            
            ...
            pre_prune_mask0, pre_prune_mask1 = (prune0 < (i + 1) / 2).squeeze(), (prune1 < (i + 1) / 2).squeeze()
...
class Lightglue_small_relposition(nn.Module):
    ...
    def __init__(self):
        ...
        self.relkenc = LearnableFourierPositionalEncoding( # KeypointEncoder(
            self.config['descriptor_dim'] // 4, self.config['descriptor_dim'] // 4)
