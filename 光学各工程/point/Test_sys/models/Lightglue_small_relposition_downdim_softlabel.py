...
def attention_assist(query, key, value, sco):
    dim = query.shape[1]
    scores = torch.einsum('bdhn,bdhm->bhnm', query, key) / dim**.5
    if sco is not None:
        scores = scores * sco[:, None, :, :]
    prob = F.softmax(scores, dim=-1)
    return torch.einsum('bhnm,bdhm->bdhn', prob, value), prob
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, encoding, sco):
        ...
        # Normal Attention
        x, _ = attention_assist(query, key, value, sco)
...
class AttentionalPropagation(nn.Module):
    ...
    def forward(self, x, source, encode, sco):
        message = self.attn(x, source, source, encode, sco)
    ...
    def forward(self, desc0, desc1, encoding0, encoding1):
        ...
        for i, (layer, name) in enumerate(zip(self.layers, self.names)):
            ...
            if name == 'cross':
                src0, src1 = desc1, desc0
                encode0, encode1 = None, None
                if len(scores0_all) > 0 and  len(scores1_all) > 0:
                    sco0 = torch.einsum('bnd,bmd->bnm', scores0_all[-1].unsqueeze(-1), scores1_all[-1].unsqueeze(-1))
                    sco1 = torch.einsum('bnd,bmd->bnm', scores1_all[-1].unsqueeze(-1), scores0_all[-1].unsqueeze(-1))    

            else:  # if name == 'self':
                src0, src1 = desc0, desc1
                encode0, encode1 = encoding0, encoding1
                if len(scores0_all) > 0 and  len(scores1_all) > 0:
                    sco0 = torch.einsum('bnd,bmd->bnm', scores0_all[-1].unsqueeze(-1), scores0_all[-1].unsqueeze(-1))
                    sco1 = torch.einsum('bnd,bmd->bnm', scores1_all[-1].unsqueeze(-1), scores1_all[-1].unsqueeze(-1))
            delta0, delta1 = layer(desc0, src0, encode0, sco0), layer(desc1, src1, encode1, sco1)
            desc0, desc1 = (desc0 + delta0), (desc1 + delta1)
            ...
            if cal_point_score:
                ...
                scores, _, scores0, scores1 = self.log_assignment[i].forward_all(desc0, desc1)
...
class Lightglue_small_relposition_downdim_softlabel(nn.Module):
    ...
    def forward(self, data):
        ...
        scores = self.gnn(desc0, desc1, encoding0, encoding1)
