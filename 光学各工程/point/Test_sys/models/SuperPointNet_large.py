"""latest version of UnSuperPointNet. Use it!
...
class SuperPointNet_large(torch.nn.Module):
    """ Pytorch definition of SuperPoint Network. """
    def __init__(self, subpixel_channel=1):
        super(SuperPointNet_large, self).__init__()
        ...
        # semi Head.
        self.semi = nn.Sequential(
            nn.Conv2d(c4, c5, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(c5),
            nn.ReLU(inplace=True),
            nn.Conv2d(c5, 65, kernel_size=1, stride=1, padding=0),
            # nn.BatchNorm2d(1),
            # nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        x1 = self.inc(x)
        # print(self.inc.conv.conv[0:3](x)[0,0,0:9,0:9])
        # exit(-1)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x4 = self.down3(x3)
        ...
        # semi head
        semi = self.semi(x4)      # [64,1,16,16]
        # semi = self.nonlinear(semi)
