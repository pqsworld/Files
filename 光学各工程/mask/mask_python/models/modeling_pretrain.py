...
__all__ = [
    'pretrain_mae_small_patch16_128', 
    'pretrain_mae_large_patch16_128', 
]
...
class PretrainVisionTransformerDecoder(nn.Module):
    ...
    def __init__(self, patch_size=16, num_classes=768, embed_dim=768, depth=12,
        ...
        super().__init__()
        self.num_classes = num_classes
        assert num_classes == patch_size ** 2
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        rec_img = rearrange(x, 'b n (p c) -> b n p c', c=1)
        rec_img = rearrange(rec_img, 'b (h w) (p1 p2) c -> b c (h p1) (w p2)', p1=16, p2=16, h=6, w=6)
        return rec_img


# @register_model
def pretrain_mae_large_patch16_128(pretrained=False, **kwargs):
...
# @register_model
def pretrain_mae_small_patch16_128(pretrained=False, **kwargs):
    model = PretrainVisionTransformer(
        img_size=128, #224
        patch_size=16,
        ...
        encoder_num_heads=6,
        encoder_num_classes=0,
        decoder_num_classes=256, # 8*8*1 64
...
if __name__ == "__main__":
    model = pretrain_mae_large_patch16_128()
    input = torch.randn(1,1,128,128)
    mask = torch.randint(2,(1,64)).bool()
