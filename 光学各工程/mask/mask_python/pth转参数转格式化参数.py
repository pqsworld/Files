...
def format_list(list,isint):
    res = ""
    if isint:
        res += "static int para_int[] = {\n"
        str_open = "w"
        str_form = "{:d},"
    else:
        res += "static int para_float[] = {\n"
        str_open = "a+"
        str_form = "{:0.7f},"

    for i in range(int(len(list) / 8) + 1):
        for j in range(8):
            index = i * 8 + j
            if index >= len(list):
                break
            # s = "{:0.7f},".format(input_param[index])
            s = str_form.format(list[index])
            res += "    {:<12}".format(s)
        res += "\n"
    # res += '#endif\n'
    res += "};\n"
    with open(dst, str_open) as f:
        f.write(res)
...
for name, module in net._modules.items():
    ...
    if isinstance(module, nn.Sequential):
        ...
        print(name)
        conv = module
        for namec,modulec in C._modules.items():
            print(namec)
            if name.find(namec)!=-1:
                conv = modulec
        if conv is not None:
            convw, convb = conv.weight.detach().numpy(
            ).flatten(), conv.bias.detach().numpy().flatten()
...
res_int = '#if 1\n'
res_int += 'static int para_int[{}] = '.format(count + 3)
res_int += '{\n'
res_int += '{},\n'.format(count + 3)
res_int += "{},\n".format(ver)
res_int += "{},\n".format(count1)
res_fp = 'static float para_float[{}] = '.format(count1 + 3)
res_fp += '{\n'
num = 0
for name, module in net._modules.items():
    if isinstance(module, nn.Sequential):
...
layersc = get_parameters_layer(C)
#print(layersc[0])
for i in range(len(layers)):
    ...
    if conv is not None:
        #print(conv)
        # print(conv.weight.detach().numpy(
        # ).ndim)
        # exit()
        convw, convb = conv.weight.detach().numpy(
        ).flatten(), conv.bias.detach().numpy().flatten()
        if convw.shape[0] % 1 == 0:
            np.savetxt('param.txt', convw.reshape(-1, 1), fmt='%1.10f',
                       delimiter=', ', newline=',\n')
        else:
        ...
        with open('param.txt', 'r') as f:
            res_fp += f.read()
        if convb.shape[0] % 1 == 0:
            np.savetxt('param.txt', convb.reshape(-1, 1), fmt='%1.10f',
                       delimiter=', ', newline=',\n')
        elif convb.shape[0] % 1 == 0:
              np.savetxt('param.txt', convb.reshape(-1, 1), fmt='%1.10f',
                        delimiter=', ', newline=',\n')
        else:
...
# res+='{}'.format(count1)
res_fp += '#endif\n'
#dst = pthpath.replace('.pth','_new.h')
...
with open(dst, 'r+') as file:
    ...
    strb = ''
    strline = strb.join(lines)
    list_para = [strline.split('{')[1],strline.split('{')[3]]

    for idx in range(len(list_para)):
        str_format = list_para[idx].split(',')
        for i in range(len(str_format)):
            if str_format[i] == '':
                str_format.remove('')
                i-=1
            if idx :
                str_format[i] = float(str_format[i])
            else:
                str_format[i] = int(str_format[i])
        print("length: "+str(len(str_format)))
        format_list(str_format,1-idx)
