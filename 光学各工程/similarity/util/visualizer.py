...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, contents=[]):
    ...
    for label, image_numpy in visuals.items():
        ...
        #     print(image_numpy)
            ...
            save_data_path = image_dir
            # print(image_dir)
            for i_na in range(len(na) - 7, len(na) - 1):
                save_data_path = save_data_path + "/" + na[i_na]
                if not os.path.isdir(save_data_path):
                    os.mkdir(save_data_path)
                    # print(save_data_path.replace('/valid/','/worsecase/'))
                    # os.mkdir(save_data_path.replace('/valid/','/worsecase/'))
            # worsepath=image_dir+'/'+na[len(na) - 7]+'/worsecase'
            # if not os.path.isdir(worsepath):
            #     os.mkdir(worsepath)
            # print(save_data_path)
            # print(image_name)
            ...
            # res=(classify_c[0][0].numpy()*255+100).astype(int)
            # gt =(gt_q[0][0].numpy()*255+100).astype(int)
            image_name = '%s_%s_%s.bmp' % (str(res),str(gt),name)
            # image_name = '%s_%s_%s.bmp' % (str((classify_c[0][0].numpy()*128+127).astype(int)),str((gt_q[0][0].numpy()*128+127).astype(int)),name)
            # image_name = '%s_%s.bmp' % (str((classify_c[0][0].numpy()*100+0.5).astype(int)),name)
            save_data_pathrcb = save_data_path+"/"+image_name
            #name_i=(int)(name.split("_")[0].split("q")[-1])
            #new_score = classify_c[0][0].numpy()*100+0.5
            #if(abs(name_i-new_score)>4):
            #img.save(save_data_pathrcb)
                #os.remove(image_path[0])
            # print(save_data_path)
            
            shutil.copy(image_path[0],save_data_pathrcb)
            # print(classify_c,gt_q,abs(classify_c-gt_q))
            # if abs(res-gt)>10:               
            #     shutil.copy(image_path[0],worsepath+'/'+image_name)
            
def save_images_mask(webpage, visuals, image_path, aspect_ratio=1.0, width=256, contents=[]):
    """Save images to the disk.
    ...
    for label, image_numpy in visuals.items():
        ...
        #     print(image_numpy)
            ...
            image_name = '%s_req_%s_gtq_%s_rec_%s_gtc_%s_%s.bmp' % (name,str((classify_c[0][0].numpy()*100+0.5).astype(int)),str((gt_q[0][0].numpy()*100+0.5).astype(int)),str((classify_c[0][1].numpy()*100+0.5).astype(int)),str((gt_c[0][0].numpy()*100+0.5).astype(int)), label)
            save_data_pathrcb = save_data_path+"/"+image_name
...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, save_result):
        ...
        #             # print(classify_c)
        #             # classify_c = int(classify_c*128+127)
        #             # classify_c = int(classify_c*355)
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            for label, image in visuals.items():
                ...
                # print(gt_c)
                # stri = '{:.3f}_{:.3f}_{:.3f}_{:.3f}'.format(classify_c[0][0].data,gt_q[0][0].data,classify_c[0][1].data,gt_c[0][0].data)
                stri = '{:.3f}_{:.3f}'.format(classify_c[0][0].data,gt_q[0][0].data)
                #print(stri)
                img_path = os.path.join(self.img_dir, 'epoch%.3d_%s_%s.png' % (epoch,stri, label))
                # print('output',image_numpy.shape)
            ...
            for n in range(epoch, 0, -1):
                ...
                for label, image_numpy in visuals.items():
                    if label == 'classify':
                        ...
                        continue
                    if label == 'gt':
                        gt_q = image_numpy#str(gt_c[0][0].numpy())
                        continue
                    if label == 'gt_c':
