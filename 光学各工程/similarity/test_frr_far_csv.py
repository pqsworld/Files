...
def _test(opt,model):
    ...
    for i, data in enumerate(dataset):
        ...
        if flag[0]=='fr':
            ...
            reslist_fr.append(res)
            gtlist_fr.append(gt)
        else:
            imglist_fa.append(img_path)
            reslist_fa.append(res)
            gtlist_fa.append(gt)
    ...
    df.to_csv(web_dir+'/out_'+opt.phase+'_fr.csv',index=False)
    ...
    df=pd.DataFrame(data)
    df.to_csv(web_dir+'/out_'+opt.phase+'_fa.csv',index=False)
    
    return reslist_fr,reslist_fa,gtlist_fr,gtlist_fa
if __name__ == '__main__':
    ...
    # print(save_data_path,'000')                           
    # import sys
    # sys.exit()
    # frr
    model.eval()
    # opt.phase='far'
    frrlist,farlist,frrgtlist,farhtlist=_test(opt,model)
    # opt.phase='frr'
    ...
    frrnumpy=np.array(frrgtlist)
    farnumpy=np.array(farhtlist)
    plt.figure()
    plt.title('gt')
    plt.hist(frrnumpy,bins=140, rwidth=0.8, range=(120,260),label='Frr', align='left',alpha=0.5)
    plt.hist(farnumpy,bins=140, rwidth=0.8, range=(120,260), label='Far', align='left',alpha=0.5)
