...
def main():
    ...
    pthpath = "/home/zhangsn/simi_net/quality_6193/checkpoints/6193simi_en2nolianxu_418/270_net_G.pth"
    ...
    img_dir = "/home/jianght/004_simi_results/2_q/outimg_q/"
    ...
    save_dir ="/data/guest/simi_data/6195_frr/"
    save_dir_65535="/data/guest/simi_data/6195_frr_65535/"
    ...
    for idx, img in enumerate(all_list):
        ...
        if net_class>65535:
            img_name_list_65535.append(img)
            net_simi_list_65535.append(pred)
            net_class_list_65535.append(net_class)  
        else:
            img_name_list_fail.append(img)
            net_simi_list_fail.append(pred)
            net_class_list_fail.append(net_class)
        ...
        if saveimg == 1:
            ...
            if net_class>65535:
                save_path=save_path.replace(img_dir, save_dir_65535)
            else:
    ...
    # 分类器匹配失败
    data={'name':img_name_list_fail,'net_simi':net_simi_list_fail,'net_class':net_class_list_fail}  #
    df=pd.DataFrame(data)
    df.to_csv('/data/guest/simi_data/out_frr_simi.csv',index=True)
    
    net_simi_list_fail= np.sort(net_simi_list_fail)
    unique_elements,counts = np.unique(net_simi_list_fail,return_counts=True)
    cal_counts = np.zeros_like(counts)
    cal_counts[0]=counts[0]
    for i in range(len(counts)-1):
        cal_counts[i+1]=cal_counts[i]+counts[i+1]
    ratio = cal_counts/(len(img_name_list_fail))*100
    data={'net_simi':unique_elements,'counts':counts,'cal_counts':cal_counts,'ratio':ratio}  #
    df=pd.DataFrame(data)
    df.to_csv('/data/guest/simi_data/out_frr_simi_counts.csv',index=True)

    # 分类器匹配成功>65535
    data={'name':img_name_list_65535,'net_simi':net_simi_list_65535,'net_class':net_class_list_65535}  #
    df=pd.DataFrame(data)
    df.to_csv('/data/guest/simi_data/out_frr_simi_65535.csv',index=True)
    
    net_simi_list_65535= np.sort(net_simi_list_65535)
    unique_elements,counts = np.unique(net_simi_list_65535,return_counts=True)
    cal_counts = np.zeros_like(counts)
    cal_counts[0]=counts[0]
    for i in range(len(counts)-1):
        cal_counts[i+1]=cal_counts[i]+counts[i+1]
    ratio = cal_counts/(len(img_name_list_65535))*100
    data={'net_simi':unique_elements,'counts':counts,'cal_counts':cal_counts,'ratio':ratio}  #
    df=pd.DataFrame(data)
    df.to_csv('/data/guest/simi_data/out_frr_simi_counts_65535.csv',index=True)
