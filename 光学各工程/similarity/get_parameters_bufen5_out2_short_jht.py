...
# pthpath = "./checkpoints/st_simi_1107_out2/291_net_G.pth"  
# pthpath = "./checkpoints/st_simi_21_1220_out2_sz72_newlabel_crop/284_net_G.pth"
# pthpath = "./checkpoints/st_simi_1205_out2_sz96_fr5_fa5_moredata_crop_tea/217_net_G.pth" 
...
block = [preconv3x3_1, preconv3x3_2, dsconv3x3_1, dsconv3x3_2, block3, block4,cls,regress]
name = []
for i in block:
    name.extend(i)

net = MNV3_bufen_new5_bin_reg(2, 1, 4, n_blocks=1)
...
for i in range(len(layers)):
    ...
    if conv is not None:
        ...
        res += 'static short para_similarity_' + \
               name[count] + \
               '_weight[{0}] = '.format(
...
res += 'static int ' + \
       'para_similarity_magnification_factor[{0}] = '.format(
...
print(count)
res += '#endif\n'
with open('st31_0109_out2_sz72_296_short.h', 'w') as f:
# with open('enhance_epoch270_simi.h', 'w') as f:
    f.write(res)
