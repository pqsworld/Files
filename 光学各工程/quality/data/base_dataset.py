    ...
    if convert:
        ...
        if "iftest" not in opt:  # the following transform only takes effect when testing
            if "c180r96" in opt:
                ...
                transform_list.append(transforms.Lambda(lambda img: __crop_180200_180__(img)))
                ...
                transform_list.append(transforms.Lambda(lambda img: __resize__tensor1(img, 96, 96)))
            elif "c160r96" in opt.name:

                transform_list += [transforms.ColorJitter(brightness=(0.8, 1.2))]  # ,contrast=(0.8,1.2),contrast=(0.8,0.8)
                # transform_list.append(angle_RandomRotation(degrees=180))
                # transform_list.append(transforms.Lambda(lambda img: block_mask(img,mask_paths)))
                transform_list.append(transforms.Lambda(lambda img: __crop_180200_160__(img)))
                # transform_list.append(__RandomColorJitter(brightness=(0.8,1.2)))   #contrast   brightness0.8-1.0   0.5-1. 2     用过brightness0.8-1.2  ,contrast=(0.8,1.2)
                transform_list.append(angle_RandomRotation_translation(degrees=20))
                # transform_list.append(transforms.Resize([96,96]))
                transform_list += [transforms.ToTensor()]
                transform_list.append(transforms.Lambda(lambda img: __resize__tensor1(img, 96, 96)))
            elif "r108c96" in opt:
                ...
                # transform_list.append(angle_RandomRotation(degrees=180))
                # transform_list.append(transforms.Lambda(lambda img: block_mask(img,mask_paths)))
                transform_list.append(transforms.Lambda(lambda img: __resize__tensor1(img, 108, 108)))
                # transform_list.append(__RandomColorJitter(brightness=(0.8,1.2)))   #contrast   brightness0.8-1.0   0.5-1. 2     用过brightness0.8-1.2  ,contrast=(0.8,1.2)
                transform_list.append(angle_RandomRotation_translation(degrees=20))
                transform_list.append(transforms.Lambda(lambda img: __crop_108_96__(img)))
                transform_list += [transforms.ToTensor()]
            else:
...
def __crop_180200_180__(image_):  # 上下各裁10
    iw, ih = image_.size
    h = ih
    w = iw
    n = 0
    img = image_.crop([n, 10, w - n, h - 10])
...
def __crop_180200_160__(image_):  # 上下各裁10

    iw, ih = image_.size
    h = ih
    w = iw
    n = 10
    img = image_.crop([n, 20, w - n, h - 20])
    # B = image_.crop([w + n, n, w*2 - n, h - n])
    img = np.array(img)
    # B = np.array(B)
    # img3 = np.hstack((img,B))
...
def __crop_108_96__(image_):  # 上下各裁10
    iw, ih = image_.size
    h = ih
    w = iw
    n = 6
    img = image_.crop([n, 6, w - n, h - 6])
    # B = image_.crop([w + n, n, w*2 - n, h - n])
    img = np.array(img)
    # B = np.array(B)
    # img3 = np.hstack((img,B))
...
class AddPepperNoise(object):
    ...
    def __call__(self, img):
        if random.uniform(0, 1) < self.p:  # 按概率进行
            ...
            mask = np.random.choice((0, 1, 2), size=(h, w, 1), p=[signal_pct, noise_pct / 2.0, noise_pct / 2.0])
            # 将mask按列复制c遍
