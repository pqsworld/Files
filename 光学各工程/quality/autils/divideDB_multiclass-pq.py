...
Date: 2024-07-19 18:00:17
LastEditors: qi.pan qi.pan@gigadevice.com
LastEditTime: 2024-07-19 18:13:33
FilePath: /vendor/autils/divideDB_multiclass.py
Description: 
...
def parse_args():
    ...
    # parser.add_argument('-p',"--path",default=r"/hdd/file-input/qint/data/oppo/cmr_new/from_ftp/内测/V6.2_0fp/", help = "DB path")
    parser.add_argument(
        "-p",
        "--path",
        default=r"/ssd/share/quality/optic_quality/datasets/nvwa-train/train",
        help="DB path",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    path = args.path
    ratio = args.r
    traindir = os.path.join(path, "train1")
    validdir = os.path.join(path, "valid1")
    ...
    for p in path.glob(r"*"):
        ...
        for root, dirs, files in os.walk(p):
            ...
            for d in dirs:
                ...
                if (
                    basePath.find("train1") != -1
                    or basePath.find("valid1") != -1
                ...
                if val_dir_num:  # 各分类至少划分n个文件夹为测试集
                    ...
                    target = target.replace("train1", "valid1")
                    val_dir_num = 0
