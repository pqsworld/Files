...
def parse_args():
    ...
    parser.add_argument('-r',"--r",default=0.1, help = "division ratio",type=float)     # 1 : 9
...
if __name__ == '__main__':
    ...
    for p in path.glob(r"*"):
        ...
        for root, dirs, files in os.walk(p):
            for f in files:
                file = Path(root)/f
                if str(file).find('.fmi')==-1 and str(file).find('.bmp')==-1:#删除非图像文件:
                    try:
                        if file.exists():
                            file.remove()
                    except:
            ...
            for d in dirs:
                ...
                if basePath.find('train')!=-1 or basePath.find('valid')!=-1 or basePath.find('!base')!=-1:
                    continue
